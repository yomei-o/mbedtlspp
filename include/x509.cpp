 ...
 */
static inline const char *mbedtls_x509_pk_type_as_string(const mbedtls_pk_context *pk)
{
 ...
 */
static inline  int x509_get_hash_alg(const mbedtls_x509_buf *alg, mbedtls_md_type_t *md_alg)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    unsigned char *p;
 ...
 */
static inline  int x509_get_attr_type_value(unsigned char **p,
                                    const unsigned char *end,
...
static inline  int x509_date_is_valid(const mbedtls_x509_time *t)
{
...
static inline  int x509_parse2_int(const unsigned char *p)
{
    uint32_t d1 = p[0] - '0';
    uint32_t d2 = p[1] - '0';
 ...
 * field.
 */
static inline  int x509_parse_time(const unsigned char *p, mbedtls_x509_time *tm,
                           size_t yearlen)
{
    int x;
...
static inline  char nibble_to_hex_digit(int i)
{
 ...
 * than size characters will be written
 */
static inline int mbedtls_x509_serial_gets(char *buf, size_t size, const mbedtls_x509_buf *serial)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t i, n, nr;
    char *p;
...
static inline int mbedtls_x509_time_cmp(const mbedtls_x509_time *t1,
...
#if defined(MBEDTLS_HAVE_TIME_DATE)
static inline int mbedtls_x509_time_gmtime(mbedtls_time_t tt, mbedtls_x509_time *now)
{
    struct tm tm;
...
static inline  int x509_get_current_time(mbedtls_x509_time *now)
{
 ...
 */
static inline  int x509_get_other_name(const mbedtls_x509_buf *subject_alt_name,
...
static inline int mbedtls_x509_parse_subject_alt_name(const mbedtls_x509_buf *san_buf,
...
static inline void mbedtls_x509_free_subject_alt_name(mbedtls_x509_subject_alternative_name *san)
{
