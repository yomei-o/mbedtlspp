...
#include <sys/stat.h>
#if defined(__MBED__)
#include <platform/mbed_retarget.hpp>
#else
#include <dirent.h>
#endif /* __MBED__ */
#include <errno.h>
...
static inline const mbedtls_x509_crt_profile mbedtls_x509_crt_profile_next =
{
    /* Hashes from SHA-256 and above. */
    MBEDTLS_X509_ID_FLAG(MBEDTLS_MD_SHA256) |
    MBEDTLS_X509_ID_FLAG(MBEDTLS_MD_SHA384) |
 ...
 * NSA Suite B Profile
 */
static inline const mbedtls_x509_crt_profile mbedtls_x509_crt_profile_suiteb =
{
 ...
 */
static inline const mbedtls_x509_crt_profile mbedtls_x509_crt_profile_none =
{
    0,
    0,
    0,
 ...
 */
static inline  int x509_profile_check_key(const mbedtls_x509_crt_profile *profile,
 ...
 */
static inline  int x509_memcasecmp(const void *s1, const void *s2, size_t len)
{
    size_t i;
    unsigned char diff;
 ...
 */
static inline  int x509_check_wildcard(const char *cn, const mbedtls_x509_buf *name)
{
    size_t i;
 ...
 */
static inline  int x509_string_cmp(const mbedtls_x509_buf *a, const mbedtls_x509_buf *b)
{
    if (a->tag == b->tag &&
        a->len == b->len &&
 ...
 * Return 0 if equal, -1 otherwise.
 */
static inline  int x509_name_cmp(const mbedtls_x509_name *a, const mbedtls_x509_name *b)
{
 ...
 */
static inline  void x509_crt_verify_chain_reset(
 ...
 */
static inline  int x509_get_version(unsigned char **p,
 ...
 *       notAfter       Time }
 */
static inline  int x509_get_dates(unsigned char **p,
                          const unsigned char *end,
 ...
 */
static inline  int x509_get_uid(unsigned char **p,
                        const unsigned char *end,
...
static inline  int x509_get_basic_constraints(unsigned char **p,
                                      const unsigned char *end,
                                      int *ca_istrue,
                                      int *max_pathlen)
{
 ...
 */
static inline  int x509_get_ext_key_usage(unsigned char **p,
                                  const unsigned char *end,
 ...
 */
static inline  int x509_get_subject_key_id(unsigned char **p,
                                   const unsigned char *end,
 ...
 *    KeyIdentifier ::= OCTET STRING
 */
static inline  int x509_get_authority_key_id(unsigned char **p,
                                     unsigned char *end,
 ...
 */
static inline  int x509_get_certificate_policies(unsigned char **p,
                                         const unsigned char *end,
 ...
 *
 */
static inline  int x509_get_crt_ext(unsigned char **p,
                            const unsigned char *end,
                            mbedtls_x509_crt *crt,
 ...
 */
static inline  int x509_crt_parse_der_core(mbedtls_x509_crt *crt,
 ...
 * chained list
 */
static inline  int mbedtls_x509_crt_parse_der_internal(mbedtls_x509_crt *chain,
                                               const unsigned char *buf,
                                               size_t buflen,
                                               int make_copy,
                                               mbedtls_x509_crt_ext_cb_t cb,
...
static inline int mbedtls_x509_crt_parse_der_nocopy(mbedtls_x509_crt *chain,
                                      const unsigned char *buf,
                                      size_t buflen)
{
...
static inline int mbedtls_x509_crt_parse_der_with_ext_cb(mbedtls_x509_crt *chain,
                                           const unsigned char *buf,
                                           size_t buflen,
                                           int make_copy,
                                           mbedtls_x509_crt_ext_cb_t cb,
...
static inline int mbedtls_x509_crt_parse_der(mbedtls_x509_crt *chain,
                               const unsigned char *buf,
                               size_t buflen)
{
 ...
 * list
 */
static inline int mbedtls_x509_crt_parse(mbedtls_x509_crt *chain,
                           const unsigned char *buf,
                           size_t buflen)
{
#if defined(MBEDTLS_PEM_PARSE_C)
 ...
 */
static inline int mbedtls_x509_crt_parse_file(mbedtls_x509_crt *chain, const char *path)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t n;
    unsigned char *buf;
...
static inline int mbedtls_x509_crt_parse_path(mbedtls_x509_crt *chain, const char *path)
{
    int ret = 0;
#if defined(_WIN32) && !defined(EFIX64) && !defined(EFI32)
    int w_ret;
...
static inline  int x509_info_ext_key_usage(char **buf, size_t *size,
...
static inline  int x509_info_cert_policies(char **buf, size_t *size,
...
static inline int mbedtls_x509_crt_info(char *buf, size_t size, const char *prefix,
...
static inline  const struct x509_crt_verify_string x509_crt_verify_strings[] = {
    MBEDTLS_X509_CRT_ERROR_INFO_LIST
    { 0, NULL }
};
#undef X509_CRT_ERROR_INFO

static inline int mbedtls_x509_crt_verify_info(char *buf, size_t size, const char *prefix,
                                 uint32_t flags)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
static inline int mbedtls_x509_crt_check_key_usage(const mbedtls_x509_crt *crt,
                                     unsigned int usage)
{
...
static inline int mbedtls_x509_crt_check_extended_key_usage(const mbedtls_x509_crt *crt,
 ...
 */
static inline int mbedtls_x509_crt_is_revoked(const mbedtls_x509_crt *crt, const mbedtls_x509_crl *crl)
{
 ...
 */
static inline  int x509_crt_verifycrl(mbedtls_x509_crt *crt, mbedtls_x509_crt *ca,
 ...
 */
static inline  int x509_crt_check_signature(const mbedtls_x509_crt *child,
 ...
 */
static inline  int x509_crt_check_parent(const mbedtls_x509_crt *child,
 ...
 */
static inline  int x509_crt_find_parent_in(
 ...
 *  - MBEDTLS_ERR_ECP_IN_PROGRESS otherwise
 */
static inline  int x509_crt_find_parent(
    mbedtls_x509_crt *child,
 ...
 */
static inline  int x509_crt_check_ee_locally_trusted(
    mbedtls_x509_crt *crt,
    mbedtls_x509_crt *trust_ca)
{
 ...
 */
static inline  int x509_crt_verify_chain(
    mbedtls_x509_crt *crt,
    mbedtls_x509_crt *trust_ca,
...
static inline  int x509_inet_pton_ipv4(const char *src, void *dst);

#define li_cton(c, n) \
    (((n) = (c) - '0') <= 9 || (((n) = ((c)&0xdf) - 'A') <= 5 ? ((n) += 10) : 0))

static inline  int x509_inet_pton_ipv6(const char *src, void *dst)
{
...
static inline size_t mbedtls_x509_crt_parse_cn_inet_pton(const char *cn, void *dst)
{
 ...
 * Check for CN match
 */
static inline  int x509_crt_check_cn(const mbedtls_x509_buf *name,
...
static inline  int x509_crt_check_san_ip(const mbedtls_x509_sequence *san,
                                 const char *cn, size_t cn_len)
{
    uint32_t ip[4];
...
static inline  int x509_crt_check_san_uri(const mbedtls_x509_sequence *san,
                                  const char *cn, size_t cn_len)
{
    for (const mbedtls_x509_sequence *cur = san; cur != NULL; cur = cur->next) {
        const unsigned char san_type = (unsigned char) cur->buf.tag &
 ...
 */
static inline  int x509_crt_check_san(const mbedtls_x509_sequence *san,
                              const char *cn, size_t cn_len)
{
    int san_ip = 0;
    int san_uri = 0;
 ...
 */
static inline  void x509_crt_verify_name(const mbedtls_x509_crt *crt,
                                 const char *cn,
                                 uint32_t *flags)
{
 ...
 */
static inline  int x509_crt_merge_flags_with_cb(
    uint32_t *flags,
 ...
 * of CRLs.
 */
static inline  int x509_crt_verify_restartable_ca_cb(mbedtls_x509_crt *crt,
                                             mbedtls_x509_crt *trust_ca,
                                             mbedtls_x509_crl *ca_crl,
                                             mbedtls_x509_crt_ca_cb_t f_ca_cb,
                                             void *p_ca_cb,
 ...
 */
static inline int mbedtls_x509_crt_verify(mbedtls_x509_crt *crt,
                            mbedtls_x509_crt *trust_ca,
                            mbedtls_x509_crl *ca_crl,
                            const char *cn, uint32_t *flags,
                            int (*f_vrfy)(void *, mbedtls_x509_crt *, int, uint32_t *),
 ...
 */
static inline int mbedtls_x509_crt_verify_with_profile(mbedtls_x509_crt *crt,
                                         mbedtls_x509_crt *trust_ca,
                                         mbedtls_x509_crl *ca_crl,
                                         const mbedtls_x509_crt_profile *profile,
                                         const char *cn, uint32_t *flags,
 ...
 *                                  not restartable).
 */
static inline int mbedtls_x509_crt_verify_with_ca_cb(mbedtls_x509_crt *crt,
                                       mbedtls_x509_crt_ca_cb_t f_ca_cb,
                                       void *p_ca_cb,
                                       const mbedtls_x509_crt_profile *profile,
                                       const char *cn, uint32_t *flags,
...
static inline int mbedtls_x509_crt_verify_restartable(mbedtls_x509_crt *crt,
                                        mbedtls_x509_crt *trust_ca,
                                        mbedtls_x509_crl *ca_crl,
                                        const mbedtls_x509_crt_profile *profile,
                                        const char *cn, uint32_t *flags,
 ...
 */
static inline void mbedtls_x509_crt_restart_init(mbedtls_x509_crt_restart_ctx *ctx)
{
 ...
 */
static inline void mbedtls_x509_crt_restart_free(mbedtls_x509_crt_restart_ctx *ctx)
{
    if (ctx == NULL) {
        return;
    }
