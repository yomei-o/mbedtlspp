 ...
 */
#if defined(HWCAP_SHA512)
static inline  int mbedtls_a64_crypto_sha512_determine_support(void)
{
...
static inline  jmp_buf return_from_sigill;

/*
 * A64 SHA512 support detection via SIGILL
 */
static inline  void sigill_handler(int signal)
{
    (void) signal;
...
static inline  void sha512_put_uint64_be(uint64_t n, unsigned char *b, uint8_t i)
{
 ...
 * Round constants
 */
static inline  const uint64_t K_2[80] =
{
...
static inline  size_t mbedtls_internal_sha512_process_many_a64_crypto(
    mbedtls_sha512_context *ctx, const uint8_t *msg, size_t len)
{
    ...
    for (;
        ...
        /* Rounds 0 and 1 */
        initial_sum = vaddq_u64(s0, vld1q_u64(&K_2[0]));
        ...
        /* Rounds 2 and 3 */
        initial_sum = vaddq_u64(s1, vld1q_u64(&K_2[2]));
        ...
        /* Rounds 4 and 5 */
        initial_sum = vaddq_u64(s2, vld1q_u64(&K_2[4]));
        ...
        /* Rounds 6 and 7 */
        initial_sum = vaddq_u64(s3, vld1q_u64(&K_2[6]));
        ...
        /* Rounds 8 and 9 */
        initial_sum = vaddq_u64(s4, vld1q_u64(&K_2[8]));
        sum = vaddq_u64(vextq_u64(initial_sum, initial_sum, 1), gh);
        intermed = vsha512hq_u64(sum, vextq_u64(ef, gh, 1), vextq_u64(cd, ef, 1));
        gh = vsha512h2q_u64(intermed, cd, ab);
        cd = vaddq_u64(cd, intermed);
        ...
        /* Rounds 10 and 11 */
        initial_sum = vaddq_u64(s5, vld1q_u64(&K_2[10]));
        sum = vaddq_u64(vextq_u64(initial_sum, initial_sum, 1), ef);
        intermed = vsha512hq_u64(sum, vextq_u64(cd, ef, 1), vextq_u64(ab, cd, 1));
        ef = vsha512h2q_u64(intermed, ab, gh);
        ab = vaddq_u64(ab, intermed);
        ...
        /* Rounds 12 and 13 */
        initial_sum = vaddq_u64(s6, vld1q_u64(&K_2[12]));
        sum = vaddq_u64(vextq_u64(initial_sum, initial_sum, 1), cd);
        intermed = vsha512hq_u64(sum, vextq_u64(ab, cd, 1), vextq_u64(gh, ab, 1));
        cd = vsha512h2q_u64(intermed, gh, ef);
        gh = vaddq_u64(gh, intermed);
        ...
        /* Rounds 14 and 15 */
        initial_sum = vaddq_u64(s7, vld1q_u64(&K_2[14]));
        sum = vaddq_u64(vextq_u64(initial_sum, initial_sum, 1), ab);
        intermed = vsha512hq_u64(sum, vextq_u64(gh, ab, 1), vextq_u64(ef, gh, 1));
        ab = vsha512h2q_u64(intermed, ef, cd);
        ef = vaddq_u64(ef, intermed);
        ...
        for (unsigned int t = 16; t < 80; t += 16) {
            ...
            initial_sum = vaddq_u64(s0, vld1q_u64(&K_2[t]));
            sum = vaddq_u64(vextq_u64(initial_sum, initial_sum, 1), gh);
            intermed = vsha512hq_u64(sum, vextq_u64(ef, gh, 1), vextq_u64(cd, ef, 1));
            gh = vsha512h2q_u64(intermed, cd, ab);
            cd = vaddq_u64(cd, intermed);
            ...
            initial_sum = vaddq_u64(s1, vld1q_u64(&K_2[t + 2]));
            sum = vaddq_u64(vextq_u64(initial_sum, initial_sum, 1), ef);
            intermed = vsha512hq_u64(sum, vextq_u64(cd, ef, 1), vextq_u64(ab, cd, 1));
            ef = vsha512h2q_u64(intermed, ab, gh);
            ab = vaddq_u64(ab, intermed);
            ...
            initial_sum = vaddq_u64(s2, vld1q_u64(&K_2[t + 4]));
            sum = vaddq_u64(vextq_u64(initial_sum, initial_sum, 1), cd);
            intermed = vsha512hq_u64(sum, vextq_u64(ab, cd, 1), vextq_u64(gh, ab, 1));
            cd = vsha512h2q_u64(intermed, gh, ef);
            gh = vaddq_u64(gh, intermed);
            ...
            initial_sum = vaddq_u64(s3, vld1q_u64(&K_2[t + 6]));
            sum = vaddq_u64(vextq_u64(initial_sum, initial_sum, 1), ab);
            intermed = vsha512hq_u64(sum, vextq_u64(gh, ab, 1), vextq_u64(ef, gh, 1));
            ab = vsha512h2q_u64(intermed, ef, cd);
            ef = vaddq_u64(ef, intermed);
            ...
            initial_sum = vaddq_u64(s4, vld1q_u64(&K_2[t + 8]));
            sum = vaddq_u64(vextq_u64(initial_sum, initial_sum, 1), gh);
            intermed = vsha512hq_u64(sum, vextq_u64(ef, gh, 1), vextq_u64(cd, ef, 1));
            gh = vsha512h2q_u64(intermed, cd, ab);
            cd = vaddq_u64(cd, intermed);
            ...
            initial_sum = vaddq_u64(s5, vld1q_u64(&K_2[t + 10]));
            sum = vaddq_u64(vextq_u64(initial_sum, initial_sum, 1), ef);
            intermed = vsha512hq_u64(sum, vextq_u64(cd, ef, 1), vextq_u64(ab, cd, 1));
            ef = vsha512h2q_u64(intermed, ab, gh);
            ab = vaddq_u64(ab, intermed);
            ...
            initial_sum = vaddq_u64(s6, vld1q_u64(&K_2[t + 12]));
            sum = vaddq_u64(vextq_u64(initial_sum, initial_sum, 1), cd);
            intermed = vsha512hq_u64(sum, vextq_u64(ab, cd, 1), vextq_u64(gh, ab, 1));
            cd = vsha512h2q_u64(intermed, gh, ef);
            gh = vaddq_u64(gh, intermed);
            ...
            initial_sum = vaddq_u64(s7, vld1q_u64(&K_2[t + 14]));
            sum = vaddq_u64(vextq_u64(initial_sum, initial_sum, 1), ab);
            intermed = vsha512hq_u64(sum, vextq_u64(gh, ab, 1), vextq_u64(ef, gh, 1));
            ab = vsha512h2q_u64(intermed, ef, cd);
            ef = vaddq_u64(ef, intermed);
...
static
#endif
static inline int mbedtls_internal_sha512_process_a64_crypto(mbedtls_sha512_context *ctx,
...
static
#endif
static inline int mbedtls_internal_sha512_process_c(mbedtls_sha512_context *ctx,
                                      const unsigned char data[SHA512_BLOCK_SIZE])
{
    int i;
    struct {
...
static inline  size_t mbedtls_internal_sha512_process_many_c(
...
static inline  int mbedtls_a64_crypto_sha512_has_support(void)
{
    static int done = 0;
...
static inline  size_t mbedtls_internal_sha512_process_many(mbedtls_sha512_context *ctx,
...
static inline int mbedtls_internal_sha512_process(mbedtls_sha512_context *ctx,
                                    const unsigned char data[SHA512_BLOCK_SIZE])
{
    if (mbedtls_a64_crypto_sha512_has_support()) {
 ...
 */
static inline  const unsigned char sha_test_buf_2[3][113] =
{
    { "abc" },
    {
...
static inline  const size_t sha_test_buflen_2[3] =
{
    3, 112, 1000
};

typedef const unsigned char (sha_test_sum_t_2)[64];

/*
 * SHA-384 test vectors
 */
#if defined(MBEDTLS_SHA384_C)
static inline  sha_test_sum_t_2 sha384_test_sum[] =
{
 ...
 */
#if defined(MBEDTLS_SHA512_C)
static inline  sha_test_sum_t_2 sha512_test_sum[] =
{
...
static inline  int mbedtls_sha512_common_self_test(int verbose, int is384)
{
    int i, buflen, ret = 0;
    unsigned char *buf;
...
#if defined(MBEDTLS_SHA384_C) && defined(MBEDTLS_SHA512_C)
    sha_test_sum_t_2 *sha_test_sum = (is384) ? sha384_test_sum : sha512_test_sum;
#elif defined(MBEDTLS_SHA512_C)
    sha_test_sum_t_2 *sha_test_sum = sha512_test_sum;
#else
    sha_test_sum_t_2 *sha_test_sum = sha384_test_sum;
#endif
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (i == 2) {
            ...
            ret = mbedtls_sha512_update(&ctx, sha_test_buf_2[i],
                                        sha_test_buflen_2[i]);
            if (ret != 0) {
                goto fail;
            }
...
#if defined(MBEDTLS_SHA512_C)
static inline int mbedtls_sha512_self_test(int verbose)
{
...
#if defined(MBEDTLS_SHA384_C)
static inline int mbedtls_sha384_self_test(int verbose)
{
