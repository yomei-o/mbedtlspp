 ...
 */
static inline void mbedtls_pkcs7_init(mbedtls_pkcs7 *pkcs7)
{
    memset(pkcs7, 0, sizeof(*pkcs7));
}

static inline  int pkcs7_get_next_content_len(unsigned char **p, unsigned char *end,
                                      size_t *len)
{
 ...
 * Version ::= INTEGER
 **/
static inline  int pkcs7_get_version(unsigned char **p, unsigned char *end, int *ver)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
 ...
 **/
static inline  int pkcs7_get_content_info_type(unsigned char **p, unsigned char *end,
 ...
 * This is from x509.h
 **/
static inline  int pkcs7_get_digest_algorithm(unsigned char **p, unsigned char *end,
 ...
 **/
static inline  int pkcs7_get_digest_algorithm_set(unsigned char **p,
                                          unsigned char *end,
                                          mbedtls_x509_buf *alg)
{
    size_t len = 0;
 ...
 **/
static inline  int pkcs7_get_certificates(unsigned char **p, unsigned char *end,
 ...
 **/
static inline  int pkcs7_get_signature(unsigned char **p, unsigned char *end,
...
static inline  void pkcs7_free_signer_info(mbedtls_pkcs7_signer_info *signer)
{
 ...
 **/
static inline  int pkcs7_get_signer_info(unsigned char **p, unsigned char *end,
 ...
 * Return negative error code for failure.
 **/
static inline  int pkcs7_get_signers_info_set(unsigned char **p, unsigned char *end,
                                      ...
                                      mbedtls_x509_buf *digest_alg)
{
    ...
    while (*p != end_set) {
        mbedtls_pkcs7_signer_info *signer =(mbedtls_pkcs7_signer_info*)
 ...
 */
static inline  int pkcs7_get_signed_data(unsigned char *buf, size_t buflen,
...
static inline int mbedtls_pkcs7_parse_der(mbedtls_pkcs7 *pkcs7, const unsigned char *buf,
                            const size_t buflen)
{
    unsigned char *p;
    unsigned char *end;
    ...
    pkcs7->raw.p = p = (unsigned char*)mbedtls_calloc(1, buflen);
    if (pkcs7->raw.p == NULL) {
        ret = MBEDTLS_ERR_PKCS7_ALLOC_FAILED;
        goto out;
    }
...
static inline  int mbedtls_pkcs7_data_or_hash_verify(mbedtls_pkcs7 *pkcs7,
                                             ...
                                             const int is_data_hash)
{
    ...
    hash = (unsigned char*)mbedtls_calloc(mbedtls_md_get_size(md_info), 1);
    if (hash == NULL) {
...
static inline int mbedtls_pkcs7_signed_data_verify(mbedtls_pkcs7 *pkcs7,
                                     const mbedtls_x509_crt *cert,
                                     const unsigned char *data,
                                     size_t datalen)
{
...
static inline int mbedtls_pkcs7_signed_hash_verify(mbedtls_pkcs7 *pkcs7,
                                     const mbedtls_x509_crt *cert,
 ...
 */
static inline void mbedtls_pkcs7_free(mbedtls_pkcs7 *pkcs7)
{
