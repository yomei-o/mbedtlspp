 ...
 */
static inline int mbedtls_aesni_has_support(unsigned int what)
{
     ...
     * Programmerfs Model for x86 Multiprocessors", CACM, 2010,
 ...
 */
static inline int mbedtls_aesni_crypt_ecb(mbedtls_aes_context *ctx,
                            int mode,
...
static inline  void gcm_clmul(const __m128i aa, const __m128i bb,
...
static inline  void gcm_shift(__m128i *cc, __m128i *dd)
{
...
static inline  __m128i gcm_reduce(__m128i xx)
{
    //                                            // xx = x1:x0
...
static inline  __m128i gcm_mix(__m128i dx)
{
...
static inline void mbedtls_aesni_gcm_mult(unsigned char c[16],
 ...
 */
#if !defined(MBEDTLS_BLOCK_CIPHER_NO_DECRYPT)
static inline void mbedtls_aesni_inverse_key(unsigned char *invkey,
 ...
 * Key expansion, 128-bit case
 */
static inline  void aesni_setkey_enc_128(unsigned char *rk,
                                 const unsigned char *key)
{
 ...
 */
#if !defined(MBEDTLS_AES_ONLY_128_BIT_KEY_LENGTH)
static inline  void aesni_setkey_enc_256(unsigned char *rk,
                                 const unsigned char *key)
{
    asm ("movdqu (%1), %%xmm0           \n\t"
         "movdqu %%xmm0, (%0)           \n\t"
 ...
 */
static inline int mbedtls_aesni_setkey_enc(unsigned char *rk,
                             const unsigned char *key,
                             size_t bits)
{
    switch (bits) {
