 ...
 */
static inline  int dhm_read_bignum(mbedtls_mpi *X,
                           unsigned char **p,
 ...
 */
static inline  int dhm_check_range(const mbedtls_mpi *param, const mbedtls_mpi *P)
{
    mbedtls_mpi U;
    int ret = 0;
...
static inline void mbedtls_dhm_init(mbedtls_dhm_context *ctx)
{
    memset(ctx, 0, sizeof(mbedtls_dhm_context));
}

static inline size_t mbedtls_dhm_get_bitlen(const mbedtls_dhm_context *ctx)
{
    return mbedtls_mpi_bitlen(&ctx->P);
}

static inline size_t mbedtls_dhm_get_len(const mbedtls_dhm_context *ctx)
{
    return mbedtls_mpi_size(&ctx->P);
}

static inline int mbedtls_dhm_get_value(const mbedtls_dhm_context *ctx,
 ...
 */
static inline int mbedtls_dhm_read_params(mbedtls_dhm_context *ctx,
                            unsigned char **p,
                            const unsigned char *end)
{
 ...
 */
static inline  int dhm_random_below(mbedtls_mpi *R, const mbedtls_mpi *M,
...
static inline  int dhm_make_common(mbedtls_dhm_context *ctx, int x_size,
 ...
 */
static inline int mbedtls_dhm_set_group(mbedtls_dhm_context *ctx,
                          const mbedtls_mpi *P,
                          const mbedtls_mpi *G)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
 ...
 */
static inline int mbedtls_dhm_read_public(mbedtls_dhm_context *ctx,
 ...
 */
static inline int mbedtls_dhm_make_public(mbedtls_dhm_context *ctx, int x_size,
                            unsigned char *output, size_t olen,
                            int (*f_rng)(void *, unsigned char *, size_t),
                            void *p_rng)
{
 ...
 */
static inline  int dhm_update_blinding(mbedtls_dhm_context *ctx,
                               int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
    int ret;
    mbedtls_mpi R;
 ...
 */
static inline int mbedtls_dhm_calc_secret(mbedtls_dhm_context *ctx,
 ...
 */
static inline void mbedtls_dhm_free(mbedtls_dhm_context *ctx)
{
    if (ctx == NULL) {
        return;
    }
 ...
 */
static inline int mbedtls_dhm_parse_dhm(mbedtls_dhm_context *dhm, const unsigned char *dhmin,
                          size_t dhminlen)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t len;
 ...
 */
static inline  int load_file(const char *path, unsigned char **buf, size_t *n)
{
    FILE *f;
    long size;
 ...
 */
static inline int mbedtls_dhm_parse_dhmfile(mbedtls_dhm_context *dhm, const char *path)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t n;
    unsigned char *buf;
...
#if defined(MBEDTLS_PEM_PARSE_C)
static inline  const char mbedtls_test_dhm_params[] =
    "-----BEGIN DH PARAMETERS-----\r\n"
...
static inline  const size_t mbedtls_test_dhm_params_len = sizeof(mbedtls_test_dhm_params);

/*
 * Checkup routine
 */
static inline int mbedtls_dhm_self_test(int verbose)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
