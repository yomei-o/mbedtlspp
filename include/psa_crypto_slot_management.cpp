...
static inline  psa_key_id_t volatile_key_id_of_index(size_t slice_idx,
                                             size_t slot_idx)
{
 ...
 * This function assumes valid parameter values. */
static inline  size_t slice_index_of_volatile_key_id(psa_key_id_t key_id)
{
 ...
 * This function assumes valid parameter values. */
static inline  size_t slot_index_of_volatile_key_id(psa_key_id_t key_id)
{
    return key_id & ((1LU << KEY_ID_SLOT_INDEX_WIDTH) - 1);
}

/* In global_data_2.first_free_slot_index, use this special value to
 * indicate that the slice is full. */
#define FREE_SLOT_INDEX_NONE ((size_t) -1)

#if defined(MBEDTLS_TEST_HOOKS)
static inline size_t psa_key_slot_volatile_slice_count(void)
{
...
} psa_global_data_2_2_t;

static inline  psa_global_data_2_2_t global_data_2;

static inline  uint8_t psa_get_key_slots_initialized(void)
{
    uint8_t initialized;
    ...
    initialized = global_data_2.key_slots_initialized;
...
#if defined(MBEDTLS_TEST_HOOKS)
static inline size_t (*mbedtls_test_hook_psa_volatile_key_slice_length)(size_t slice_idx) = NULL;
#endif
...
static inline  psa_key_slot_t *get_volatile_key_slot(psa_key_id_t key_id)
{
    ...
    psa_key_slot_t *slice = global_data_2.key_slices[slice_idx];
    if (slice == NULL) {
        return NULL;
    }
...
static inline  psa_key_slot_t *get_persistent_key_slot(size_t slot_idx)
{
    return &global_data_2.key_slices[KEY_SLOT_CACHE_SLICE_INDEX][slot_idx];
}

static inline  psa_key_slot_t *get_key_slot(size_t slice_idx, size_t slot_idx)
{
    return &global_data_2.key_slices[slice_idx][slot_idx];
}
...
{
    (void) slice_idx;
    return ARRAY_LENGTH(global_data_2.key_slots);
}

static inline  psa_key_slot_t *get_volatile_key_slot(psa_key_id_t key_id)
{
    MBEDTLS_STATIC_ASSERT(ARRAY_LENGTH(global_data_2.key_slots) <=
                          PSA_KEY_ID_VOLATILE_MAX - PSA_KEY_ID_VOLATILE_MIN + 1,
                          "The key slot array is larger than the volatile key ID range");
    return &global_data_2.key_slots[key_id - PSA_KEY_ID_VOLATILE_MIN];
}

static inline  psa_key_slot_t *get_persistent_key_slot(size_t slot_idx)
{
    return &global_data_2.key_slots[slot_idx];
}
...
static inline int psa_is_valid_key_id(mbedtls_svc_key_id_t key, int vendor_ok)
{
...
{
#if defined(MBEDTLS_PSA_KEY_STORE_DYNAMIC)
    global_data_2.key_slices[KEY_SLOT_CACHE_SLICE_INDEX] =(psa_key_slot_t*)
        mbedtls_calloc(PERSISTENT_KEY_CACHE_COUNT,
                       sizeof(*global_data_2.key_slices[KEY_SLOT_CACHE_SLICE_INDEX]));
    if (global_data_2.key_slices[KEY_SLOT_CACHE_SLICE_INDEX] == NULL) {
    ...
    global_data_2.key_slots_initialized = 1;
    return PSA_SUCCESS;
}
...
#if defined(MBEDTLS_PSA_KEY_STORE_DYNAMIC)
        if (global_data_2.key_slices[slice_idx] == NULL) {
            continue;
        }
#endif  /* MBEDTLS_PSA_KEY_STORE_DYNAMIC */
...
#if defined(MBEDTLS_PSA_KEY_STORE_DYNAMIC)
        mbedtls_free(global_data_2.key_slices[slice_idx]);
        global_data_2.key_slices[slice_idx] = NULL;
#endif  /* MBEDTLS_PSA_KEY_STORE_DYNAMIC */
    }

#if defined(MBEDTLS_PSA_KEY_STORE_DYNAMIC)
    for (size_t slice_idx = 0; slice_idx < KEY_SLOT_VOLATILE_SLICE_COUNT; slice_idx++) {
        global_data_2.first_free_slot_index[slice_idx] = 0;
    }
#endif  /* MBEDTLS_PSA_KEY_STORE_DYNAMIC */

    /* The global data mutex is already held when calling this function. */
    global_data_2.key_slots_initialized = 0;
}

#if defined(MBEDTLS_PSA_KEY_STORE_DYNAMIC)

static inline  psa_status_t psa_allocate_volatile_key_slot(psa_key_id_t *key_id,
                                                   psa_key_slot_t **p_slot)
{
    size_t slice_idx;
    for (slice_idx = 0; slice_idx < KEY_SLOT_VOLATILE_SLICE_COUNT; slice_idx++) {
        if (global_data_2.first_free_slot_index[slice_idx] != FREE_SLOT_INDEX_NONE) {
            break;
        }
    }
    if (slice_idx == KEY_SLOT_VOLATILE_SLICE_COUNT) {
    ...
    if (global_data_2.key_slices[slice_idx] == NULL) {
        global_data_2.key_slices[slice_idx] =(psa_key_slot_t*)
    ...
    size_t slot_idx = global_data_2.first_free_slot_index[slice_idx];
    *key_id = volatile_key_id_of_index(slice_idx, slot_idx);
    ...
    global_data_2.first_free_slot_index[slice_idx] = next_free;
...
static inline psa_status_t psa_free_key_slot(size_t slice_idx,
                               psa_key_slot_t *slot)
{
    ...
    size_t next_free = global_data_2.first_free_slot_index[slice_idx];
    if (next_free >= key_slice_length(slice_idx)) {
        /* The slot was full. The newly freed slot thus becomes the
         * end of the free list. */
        next_free = key_slice_length(slice_idx);
    }
    global_data_2.first_free_slot_index[slice_idx] = slot_idx;
...
static inline psa_status_t psa_reserve_free_key_slot(psa_key_id_t *volatile_key_id,
                                       psa_key_slot_t **p_slot)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    size_t slot_idx;
    ...
    if (selected_slot != NULL) {
        ...
        if (volatile_key_id != NULL) {
            ...
            slot_idx = selected_slot - global_data_2.key_slots;
...
static inline psa_status_t psa_open_key(mbedtls_svc_key_id_t key, psa_key_handle_t *handle)
{
#if defined(MBEDTLS_PSA_CRYPTO_STORAGE_C) || \
    defined(MBEDTLS_PSA_CRYPTO_BUILTIN_KEYS)
    psa_status_t status;
...
static inline psa_status_t psa_close_key(psa_key_handle_t handle)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    psa_key_slot_t *slot;
...
static inline void mbedtls_psa_get_stats(mbedtls_psa_stats_t *stats)
{
