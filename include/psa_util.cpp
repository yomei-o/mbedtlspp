...
#if defined(MBEDTLS_MD_LIGHT)
static inline const mbedtls_error_pair_t psa_to_md_errors[] =
{
    { PSA_SUCCESS,                     0 },
...
#if defined(MBEDTLS_BLOCK_CIPHER_SOME_PSA)
static inline const mbedtls_error_pair_t psa_to_cipher_errors[] =
{
    { PSA_SUCCESS,                     0 },
...
#if defined(MBEDTLS_LMS_C)
static inline const mbedtls_error_pair_t psa_to_lms_errors[] =
{
    { PSA_SUCCESS,                     0 },
...
#if defined(MBEDTLS_SSL_TLS_C) && \
    (defined(MBEDTLS_USE_PSA_CRYPTO) || defined(MBEDTLS_SSL_PROTO_TLS1_3))
static inline const mbedtls_error_pair_t psa_to_ssl_errors[] =
{
    { PSA_SUCCESS,                     0 },
...
#if defined(PSA_WANT_KEY_TYPE_RSA_PUBLIC_KEY) ||    \
    defined(PSA_WANT_KEY_TYPE_RSA_KEY_PAIR_BASIC)
static inline const mbedtls_error_pair_t psa_to_pk_rsa_errors[] =
{
    { PSA_SUCCESS,                     0 },
...
#if defined(MBEDTLS_USE_PSA_CRYPTO) && \
    defined(PSA_WANT_KEY_TYPE_ECC_PUBLIC_KEY)
static inline const mbedtls_error_pair_t psa_to_pk_ecdsa_errors[] =
{
    { PSA_SUCCESS,                     0 },
...
static inline int psa_generic_status_to_mbedtls(psa_status_t status)
{
    switch (status) {
        case PSA_SUCCESS:
            return 0;
...
#if defined(MBEDTLS_PK_C)
static inline int psa_pk_status_to_mbedtls(psa_status_t status)
{
    switch (status) {
...
static inline mbedtls_ecp_group_id mbedtls_ecc_group_from_psa(psa_ecc_family_t family,
                                                size_t bits)
{
    switch (family) {
 ...
 */
static inline  int convert_raw_to_der_single_int(const unsigned char *raw_buf, size_t raw_len,
...
static inline int mbedtls_ecdsa_raw_to_der(size_t bits, const unsigned char *raw, size_t raw_len,
 ...
 */
static inline  int convert_der_to_raw_single_int(unsigned char *der, size_t der_len,
...
static inline int mbedtls_ecdsa_der_to_raw(size_t bits, const unsigned char *der, size_t der_len,
