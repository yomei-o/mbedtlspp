...
static inline  const x509_attr_descriptor_t x509_attrs[] =
{
    { ADD_STRLEN("CN"),
...
static inline  const x509_attr_descriptor_t *x509_attr_descr_from_name(const char *name, size_t name_len)
{
...
static inline  int hexpair_to_int(const char *hexpair)
{
...
static inline  int parse_attribute_value_string(const char *s,
                                        int len,
                                        unsigned char *data,
                                        size_t *data_len)
{
 ...
 */
static inline  int parse_attribute_value_hex_der_encoded(const char *s,
                                                 size_t len,
                                                 unsigned char *data,
                                                 size_t data_size,
                                                 size_t *data_len,
...
static inline int mbedtls_x509_string_to_names(mbedtls_asn1_named_data **head, const char *name)
{
    int ret = MBEDTLS_ERR_X509_INVALID_NAME;
    int parse_ret = 0;
    const char *s = name, *c = s;
    const char *end = s + strlen(s);
    mbedtls_asn1_buf oid = { .tag = MBEDTLS_ASN1_NULL,.len = 0, .p = NULL};
    ...
    while (c <= end) {
        if (in_attr_type && *c == '=') {
            if ((attr_descr = x509_attr_descr_from_name(s, (size_t) (c - s))) == NULL) {
                ...
                oid.p = (unsigned char *) mbedtls_calloc(1, oid.len);
 ...
 */
static inline  int x509_write_name(unsigned char **p,
                           unsigned char *start,
...
static inline  int x509_write_extension(unsigned char **p, unsigned char *start,
