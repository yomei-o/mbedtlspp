...
static inline  int debug_threshold = 0;

static inline void mbedtls_debug_set_threshold(int threshold)
{
...
static inline void mbedtls_debug_print_msg(const mbedtls_ssl_context *ssl, int level,
                             const char *file, int line,
                             const char *format, ...)
{
    va_list argp;
...
static inline void mbedtls_debug_print_ret(const mbedtls_ssl_context *ssl, int level,
                             const char *file, int line,
...
static inline void mbedtls_debug_print_buf(const mbedtls_ssl_context *ssl, int level,
...
static inline void mbedtls_debug_print_mpi(const mbedtls_ssl_context *ssl, int level,
                             const char *file, int line,
...
static inline  void mbedtls_debug_print_integer(const mbedtls_ssl_context *ssl, int level,
                                        const char *file, int line, const char *text,
...
static inline  void mbedtls_debug_print_psa_ec(const mbedtls_ssl_context *ssl, int level,
                                       const char *file, int line,
...
static inline  size_t debug_count_valid_bits(unsigned char **buf, size_t len)
{
    size_t i, bits;
...
static inline  void mbedtls_debug_print_psa_rsa(const mbedtls_ssl_context *ssl, int level,
                                        const char *file, int line,
                                        const char *text, const mbedtls_pk_context *pk)
{
    char str[DEBUG_BUF_SIZE];
...
static inline  void debug_print_pk(const mbedtls_ssl_context *ssl, int level,
                           const char *file, int line,
                           const char *text, const mbedtls_pk_context *pk)
{
    size_t i;
...
/* no-check-names will be removed in mbedtls#10229. */
        ...
        if (items[i].type == MBEDTLS_PK_DEBUG_PSA_RSA) { //no-check-names
            mbedtls_debug_print_psa_rsa(ssl, level, file, line, name, (const mbedtls_pk_context *)items[i].value);
        } else
#endif /* MBEDTLS_PK_USE_PSA_RSA_DATA */ //no-check-names
#if defined(PSA_WANT_KEY_TYPE_ECC_PUBLIC_KEY)
        if (items[i].type == MBEDTLS_PK_DEBUG_PSA_EC) {
            mbedtls_debug_print_psa_ec(ssl, level, file, line, name, (const mbedtls_pk_context *)items[i].value);
        } else
#endif /* PSA_WANT_KEY_TYPE_ECC_PUBLIC_KEY */
        { debug_send_line(ssl, level, file, line,
                          "should not happen\n"); }
...
static inline  void debug_print_line_by_line(const mbedtls_ssl_context *ssl, int level,
                                     const char *file, int line, const char *text)
{
    char str[DEBUG_BUF_SIZE];
...
static inline void mbedtls_debug_print_crt(const mbedtls_ssl_context *ssl, int level,
                             const char *file, int line,
