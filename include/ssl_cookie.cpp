...
#include "mbedtls_ssl_cookie.hpp"
...
static inline  int local_err_translation_6(psa_status_t status)
{
    return psa_status_to_mbedtls(status, psa_to_ssl_errors,
                                 ARRAY_LENGTH(psa_to_ssl_errors),
                                 psa_generic_status_to_mbedtls);
}
#define PSA_TO_MBEDTLS_ERR(status) local_err_translation_6(status)
...
static inline void mbedtls_ssl_cookie_init(mbedtls_ssl_cookie_ctx *ctx)
{
...
static inline void mbedtls_ssl_cookie_set_timeout(mbedtls_ssl_cookie_ctx *ctx, unsigned long delay)
{
    ctx->timeout = delay;
}

static inline void mbedtls_ssl_cookie_free(mbedtls_ssl_cookie_ctx *ctx)
{
    if (ctx == NULL) {
        return;
    }
...
static inline int mbedtls_ssl_cookie_setup(mbedtls_ssl_cookie_ctx *ctx)
{
 ...
 */
static inline int mbedtls_ssl_cookie_write(void *p_ctx,
 ...
 * Check a cookie
 */
static inline int mbedtls_ssl_cookie_check(void *p_ctx,
