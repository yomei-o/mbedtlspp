...
#include "mbedtls_private_poly1305.hpp"
 ...
 */
static inline  void poly1305_process(mbedtls_poly1305_context *ctx,
                             size_t nblocks,
 ...
 */
static inline  void poly1305_compute_mac(const mbedtls_poly1305_context *ctx,
...
static inline void mbedtls_poly1305_init(mbedtls_poly1305_context *ctx)
{
    mbedtls_platform_zeroize(ctx, sizeof(mbedtls_poly1305_context));
}

static inline void mbedtls_poly1305_free(mbedtls_poly1305_context *ctx)
{
    if (ctx == NULL) {
        return;
    }
...
static inline int mbedtls_poly1305_starts(mbedtls_poly1305_context *ctx,
...
static inline int mbedtls_poly1305_update(mbedtls_poly1305_context *ctx,
                            const unsigned char *input,
                            size_t ilen)
{
    size_t offset    = 0U;
...
static inline int mbedtls_poly1305_finish(mbedtls_poly1305_context *ctx,
                            unsigned char mac[16])
{
...
static inline int mbedtls_poly1305_mac(const unsigned char key[32],
                         const unsigned char *input,
                         size_t ilen,
                         unsigned char mac[16])
{
...
static inline  const unsigned char test_keys_2[2][32] =
{
    {
...
static inline  const unsigned char test_data[2][127] =
{
    {
...
static inline  const size_t test_data_len[2] =
{
    34U,
    127U
};

static inline  const unsigned char test_mac_2[2][16] =
{
    {
...
static inline int mbedtls_poly1305_self_test(int verbose)
{
    unsigned char mac[16];
    unsigned i;
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    for (i = 0U; i < 2U; i++) {
        ...
        ret = mbedtls_poly1305_mac(test_keys_2[i],
                                   test_data[i],
                                   test_data_len[i],
                                   mac);
        ASSERT(0 == ret, ("error code: %i\n", ret));

        ASSERT(0 == memcmp(mac, test_mac_2[i], 16U), ("failed (mac)\n"));
