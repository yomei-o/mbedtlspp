...
static inline  int local_err_translation_3(psa_status_t status)
{
 ...
 */
//yomei
static inline  const char tls13_label_prefix[7] = "tls13 ";
#define sizeof_tls13_label_prefix 6
...
#define SSL_TLS1_3_KEY_SCHEDULE_MAX_HKDF_LABEL_LEN                      \
    SSL_TLS1_3_KEY_SCHEDULE_HKDF_LABEL_LEN(                             \
        sizeof_tls13_label_prefix +                       \
        MBEDTLS_SSL_TLS1_3_HKDF_LABEL_MAX_LABEL_LEN,       \
        MBEDTLS_SSL_TLS1_3_KEY_SCHEDULE_MAX_CONTEXT_LEN)

static inline  void ssl_tls13_hkdf_encode_label(
    ...
    unsigned char *dst, size_t *dst_len)
{
    size_t total_label_len =
        sizeof_tls13_label_prefix + label_len;
...
#error "The desired key length must fit into an uint16 but \
    ...
    *p++ = MBEDTLS_BYTE_0(total_label_len);
    memcpy(p, tls13_label_prefix, sizeof_tls13_label_prefix);
    p += sizeof_tls13_label_prefix;
...
static inline int mbedtls_ssl_tls13_hkdf_expand_label(
...
static inline  int ssl_tls13_make_traffic_key(
    psa_algorithm_t hash_alg,
    const unsigned char *secret, size_t secret_len,
 ...
 */
static inline int mbedtls_ssl_tls13_make_traffic_keys(
    psa_algorithm_t hash_alg,
...
static inline int mbedtls_ssl_tls13_derive_secret(
    psa_algorithm_t hash_alg,
    const unsigned char *secret, size_t secret_len,
    const unsigned char *label, size_t label_len,
    const unsigned char *ctx, size_t ctx_len,
...
static inline int mbedtls_ssl_tls13_evolve_secret(
    psa_algorithm_t hash_alg,
...
static inline int mbedtls_ssl_tls13_derive_early_secrets(
    psa_algorithm_t hash_alg,
...
static inline int mbedtls_ssl_tls13_derive_handshake_secrets(
    psa_algorithm_t hash_alg,
...
static inline int mbedtls_ssl_tls13_derive_application_secrets(
    psa_algorithm_t hash_alg,
...
static inline int mbedtls_ssl_tls13_derive_resumption_master_secret(
    psa_algorithm_t hash_alg,
    unsigned char const *application_secret,
    unsigned char const *transcript, size_t transcript_len,
    mbedtls_ssl_tls13_application_secrets *derived)
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_tls13_key_schedule_stage_application(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_tls13_calc_finished_core(psa_algorithm_t hash_alg,
...
static inline int mbedtls_ssl_tls13_calculate_verify_data(mbedtls_ssl_context *ssl,
                                            unsigned char *dst,
                                            size_t dst_len,
                                            size_t *actual_len,
                                            int from)
...
static inline int mbedtls_ssl_tls13_create_psk_binder(mbedtls_ssl_context *ssl,
...
static inline int mbedtls_ssl_tls13_populate_transform(
...
#if !defined(MBEDTLS_USE_PSA_CRYPTO)
    cipher_info = mbedtls_cipher_info_from_type((const mbedtls_cipher_type_t)ciphersuite_info->cipher);
    if (cipher_info == NULL) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_tls13_get_cipher_key_info(
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info,
    size_t *key_len, size_t *iv_len)
{
    psa_key_type_t key_type;
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_tls13_generate_early_key(mbedtls_ssl_context *ssl,
...
static inline int mbedtls_ssl_tls13_compute_early_transform(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_ssl_key_set traffic_keys;
...
static inline int mbedtls_ssl_tls13_key_schedule_stage_early(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    psa_algorithm_t hash_alg;
    mbedtls_ssl_handshake_params *handshake = ssl->handshake;
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_tls13_generate_handshake_keys(mbedtls_ssl_context *ssl,
                                             mbedtls_ssl_key_set *traffic_keys)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_md_type_t md_type;
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_tls13_key_schedule_stage_handshake(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_ssl_handshake_params *handshake = ssl->handshake;
    psa_algorithm_t const hash_alg = mbedtls_md_psa_alg_from_type(
    ...
    if (mbedtls_ssl_tls13_key_exchange_mode_with_ephemeral(ssl)) {
        if (mbedtls_ssl_tls13_named_group_is_ecdhe(handshake->offered_group_id) ||
            ...
            status = psa_get_key_attributes(handshake->xxdh_psa_privkey,
                                            &key_attributes);
            if (status != PSA_SUCCESS) {
                ret = (int)PSA_TO_MBEDTLS_ERR(status);
            }

            shared_secret_len = PSA_BITS_TO_BYTES(
                psa_get_key_bits(&key_attributes));
            shared_secret = (unsigned char*)mbedtls_calloc(1, shared_secret_len);
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_tls13_generate_application_keys(
...
static inline int mbedtls_ssl_tls13_compute_handshake_transform(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_ssl_key_set traffic_keys;
    ...
    transform_handshake = (mbedtls_ssl_transform*)mbedtls_calloc(1, sizeof(mbedtls_ssl_transform));
...
static inline int mbedtls_ssl_tls13_compute_resumption_master_secret(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_md_type_t md_type;
    mbedtls_ssl_handshake_params *handshake = ssl->handshake;
...
static inline int mbedtls_ssl_tls13_compute_application_transform(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_ssl_key_set traffic_keys;
    ...
    transform_application =(mbedtls_ssl_transform*)
...
#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_PSK_ENABLED)
static inline int mbedtls_ssl_tls13_export_handshake_psk(mbedtls_ssl_context *ssl,
                                           unsigned char **psk,
                                           size_t *psk_len)
{
#if defined(MBEDTLS_USE_PSA_CRYPTO)
...
static inline int mbedtls_ssl_tls13_exporter(const psa_algorithm_t hash_alg,
