...
#include "block_cipher_internal.hpp"
#endif
 ...
 */
static inline  int gcm_gen_table(mbedtls_gcm_context *ctx)
{
    int ret, i, j;
    uint64_t u64h[2] = { 0 };
...
#if defined(MBEDTLS_GCM_LARGE_TABLE)
static inline  const uint16_t last8[256] = {
...
static inline  void gcm_mult_largetable(uint8_t *output, const uint8_t *x, uint64_t H[256][2])
{
    int i;
    uint64_t u64z[2];
 ...
 */
static inline  const uint16_t last4[16] =
{
...
static inline  void gcm_mult_smalltable(uint8_t *output, const uint8_t *x, uint64_t H[16][2])
{
    int i = 0;
 ...
 */
static inline  void gcm_mult(mbedtls_gcm_context *ctx, const unsigned char x[16],
 ...
 *                                      the data ends now.
 */
static inline int mbedtls_gcm_update_ad(mbedtls_gcm_context *ctx,
...
static inline  void gcm_incr(unsigned char y[16])
{
...
static inline  int gcm_mask(mbedtls_gcm_context *ctx,
...
static inline int mbedtls_gcm_crypt_and_tag(mbedtls_gcm_context *ctx,
                              int mode,
                              size_t length,
...
static inline int mbedtls_gcm_auth_decrypt(mbedtls_gcm_context *ctx,
                             size_t length,
                             const unsigned char *iv,
                             size_t iv_len,
                             const unsigned char *add,
...
static inline  const int key_index_test_data[MAX_TESTS] =
{ 0, 0, 1, 1, 1, 1 };

static inline  const unsigned char key_test_data_2[][32] =
{
...
static inline  const size_t iv_len_test_data_2[MAX_TESTS] =
{ 12, 12, 12, 12, 8, 60 };

static inline  const int iv_index_test_data[MAX_TESTS] =
{ 0, 0, 1, 1, 1, 2 };

static inline  const unsigned char iv_test_data_2[][64] =
{
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00 },
...
static inline  const size_t add_len_test_data_2[MAX_TESTS] =
{ 0, 0, 0, 20, 20, 20 };

static inline  const int add_index_test_data[MAX_TESTS] =
{ 0, 0, 0, 1, 1, 1 };

static inline  const unsigned char additional_test_data[][64] =
{
    { 0x00 },
...
static inline  const size_t pt_len_test_data[MAX_TESTS] =
{ 0, 16, 64, 60, 60, 60 };

static inline  const int pt_index_test_data[MAX_TESTS] =
{ 0, 0, 1, 1, 1, 1 };

static inline  const unsigned char pt_test_data[][64] =
{
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },
...
static inline  const unsigned char ct_test_data[][64] =
{
    { 0x00 },
...
static inline  const unsigned char tag_test_data[][16] =
{
...
static inline int mbedtls_gcm_self_test(int verbose)
{
    ...
    for (j = 0; j < loop_limit; j++) {
        ...
        for (i = 0; i < MAX_TESTS; i++) {
            ...
            ret = mbedtls_gcm_setkey(&ctx, cipher,
                                     key_test_data_2[key_index_test_data[i]],
                                     key_len);
            /*
            ...
            ret = mbedtls_gcm_crypt_and_tag(&ctx, MBEDTLS_GCM_ENCRYPT,
                                            pt_len_test_data[i],
                                            iv_test_data_2[iv_index_test_data[i]],
                                            iv_len_test_data_2[i],
                                            additional_test_data[add_index_test_data[i]],
                                            add_len_test_data_2[i],
                                            pt_test_data[pt_index_test_data[i]],
                                            buf, 16, tag_buf);
#if defined(MBEDTLS_GCM_ALT)
            /* Allow alternative implementations to only support 12-byte nonces. */
            if (ret == MBEDTLS_ERR_PLATFORM_FEATURE_UNSUPPORTED &&
                iv_len_test_data_2[i] != 12) {
                mbedtls_printf("skipped\n");
                break;
            }
