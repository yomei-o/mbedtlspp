...
#include "psa/crypto_se_driver.hpp"

#include "psa_crypto_se.hpp"

#if defined(MBEDTLS_PSA_ITS_FILE_C)
#include "psa_crypto_its.hpp"
#else /* Native ITS implementation */
#include "psa/error.hpp"
#include "psa/internal_trusted_storage.hpp"
#endif
...
static inline  psa_se_drv_table_entry_t driver_table[PSA_MAX_SE_DRIVERS];

static inline psa_se_drv_table_entry_t *psa_get_se_driver_entry(
...
static inline const psa_drv_se_t *psa_get_se_driver_methods(
    const psa_se_drv_table_entry_t *driver)
{
    return driver->methods;
}

static inline psa_drv_se_context_t *psa_get_se_driver_context(
    psa_se_drv_table_entry_t *driver)
{
    return &driver->u.context;
}

static inline int psa_get_se_driver(psa_key_lifetime_t lifetime,
...
static inline  psa_status_t psa_get_se_driver_its_file_uid(
    const psa_se_drv_table_entry_t *driver,
...
static inline psa_status_t psa_load_se_persistent_data(
    const psa_se_drv_table_entry_t *driver)
{
    psa_status_t status;
    psa_storage_uid_t uid;
...
static inline psa_status_t psa_save_se_persistent_data(
    const psa_se_drv_table_entry_t *driver)
{
    psa_status_t status;
    psa_storage_uid_t uid;
...
static inline psa_status_t psa_destroy_se_persistent_data(psa_key_location_t location)
{
    psa_storage_uid_t uid;
...
static inline psa_status_t psa_find_se_slot_for_key(
...
static inline psa_status_t psa_destroy_se_key(psa_se_drv_table_entry_t *driver,
                                psa_key_slot_number_t slot_number)
{
    psa_status_t status;
...
static inline psa_status_t psa_init_all_se_drivers(void)
{
    size_t i;
    for (i = 0; i < PSA_MAX_SE_DRIVERS; i++) {
...
static inline psa_status_t psa_register_se_driver(
    psa_key_location_t location,
    const psa_drv_se_t *methods)
{
    size_t i;
...
static inline void psa_unregister_all_se_drivers(void)
{
    size_t i;
    for (i = 0; i < PSA_MAX_SE_DRIVERS; i++) {
        if (driver_table[i].u.internal.persistent_data != NULL) {
