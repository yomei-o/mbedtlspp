...
#include "mbedtls_ssl_ticket.hpp"
...
static inline  int local_err_translation_8(psa_status_t status)
{
    return psa_status_to_mbedtls(status, psa_to_ssl_errors,
                                 ARRAY_LENGTH(psa_to_ssl_errors),
                                 psa_generic_status_to_mbedtls);
}
#define PSA_TO_MBEDTLS_ERR(status) local_err_translation_8(status)

/*
 * Initialize context
 */
static inline void mbedtls_ssl_ticket_init(mbedtls_ssl_ticket_context *ctx)
{
...
static inline  int ssl_ticket_gen_key(mbedtls_ssl_ticket_context *ctx,
                              unsigned char index)
{
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_ticket_update_keys(mbedtls_ssl_ticket_context *ctx)
{
#if !defined(MBEDTLS_HAVE_TIME)
    ((void) ctx);
#else
 ...
 */
static inline int mbedtls_ssl_ticket_rotate(mbedtls_ssl_ticket_context *ctx,
...
static inline int mbedtls_ssl_ticket_write(void *p_ticket,
 ...
 */
static inline  mbedtls_ssl_ticket_key *ssl_ticket_select_key(
 ...
 */
static inline int mbedtls_ssl_ticket_parse(void *p_ticket,
...
#if defined(MBEDTLS_HAVE_TIME)
    /* Declared up front: the assignments below are crossed by `goto cleanup`,
     * which C++ does not allow to jump over an initialiser. */
...
#if defined(MBEDTLS_HAVE_TIME)
	//yomei
	ticket_lifetime =(mbedtls_ms_time_t) key->lifetime * 1000;
 ...
 * Free context
 */
static inline void mbedtls_ssl_ticket_free(mbedtls_ssl_ticket_context *ctx)
{
    if (ctx == NULL) {
        return;
    }
