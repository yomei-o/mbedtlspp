 ...
 * S-Boxes
 */
static inline  const uint8_t aria_sb1[256] =
{
...
static inline  const uint8_t aria_sb2[256] =
{
...
static inline  const uint8_t aria_is1[256] =
{
...
static inline  const uint8_t aria_is2[256] =
{
 ...
 */
static inline  void aria_fo_xor(uint32_t r[4], const uint32_t p[4],
 ...
 */
static inline  void aria_fe_xor(uint32_t r[4], const uint32_t p[4],
                        const uint32_t k[4], const uint32_t x[4])
{
    uint32_t a, b, c, d;
 ...
 * bytes here.
 */
static inline  void aria_rot128(uint32_t r[4], const uint32_t a[4],
 ...
 * Set encryption key
 */
static inline int mbedtls_aria_setkey_enc(mbedtls_aria_context *ctx,
...
static inline int mbedtls_aria_setkey_dec(mbedtls_aria_context *ctx,
                            const unsigned char *key, unsigned int keybits)
{
    int i, j, k, ret;
 ...
 * Encrypt a block
 */
static inline int mbedtls_aria_crypt_ecb(mbedtls_aria_context *ctx,
...
/* Initialize context */
static inline void mbedtls_aria_init(mbedtls_aria_context *ctx)
{
    memset(ctx, 0, sizeof(mbedtls_aria_context));
}

/* Clear context */
static inline void mbedtls_aria_free(mbedtls_aria_context *ctx)
{
    if (ctx == NULL) {
        return;
    }
 ...
 */
static inline int mbedtls_aria_crypt_cbc(mbedtls_aria_context *ctx,
                           int mode,
                           size_t length,
 ...
 */
static inline int mbedtls_aria_crypt_cfb128(mbedtls_aria_context *ctx,
                              int mode,
                              size_t length,
                              size_t *iv_off,
                              unsigned char iv[MBEDTLS_ARIA_BLOCKSIZE],
 ...
 */
static inline int mbedtls_aria_crypt_ctr(mbedtls_aria_context *ctx,
                           size_t length,
                           size_t *nc_off,
 ...
 */
static inline  const uint8_t aria_test1_ecb_key[32] = // test key
{
...
static inline  const uint8_t aria_test1_ecb_pt[MBEDTLS_ARIA_BLOCKSIZE] = // plaintext
{
    0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,     // same for all
    0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF      // key sizes
};

static inline  const uint8_t aria_test1_ecb_ct[3][MBEDTLS_ARIA_BLOCKSIZE] = // ciphertext
{
...
static inline  const uint8_t aria_test2_key[32] =
{
...
static inline  const uint8_t aria_test2_pt[48] =
{
...
#if (defined(MBEDTLS_CIPHER_MODE_CBC) || defined(MBEDTLS_CIPHER_MODE_CFB))
static inline  const uint8_t aria_test2_iv[MBEDTLS_ARIA_BLOCKSIZE] =
{
...
#if defined(MBEDTLS_CIPHER_MODE_CBC)
static inline  const uint8_t aria_test2_cbc_ct[3][48] = // CBC ciphertext
{
...
#if defined(MBEDTLS_CIPHER_MODE_CFB)
static inline  const uint8_t aria_test2_cfb_ct[3][48] = // CFB ciphertext
{
...
#if defined(MBEDTLS_CIPHER_MODE_CTR)
static inline  const uint8_t aria_test2_ctr_ct[3][48] = // CTR ciphertext
{
 ...
 * Checkup routine
 */
static inline int mbedtls_aria_self_test(int verbose)
{
    int i;
