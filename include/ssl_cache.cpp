...
static inline void mbedtls_ssl_cache_init(mbedtls_ssl_cache_context *cache)
{
...
static inline  int ssl_cache_find_entry(mbedtls_ssl_cache_context *cache,
...
static inline int mbedtls_ssl_cache_get(void *data,
                          unsigned char const *session_id,
                          size_t session_id_len,
...
static inline  void ssl_cache_entry_zeroize(mbedtls_ssl_cache_entry *entry)
{
    if (entry == NULL) {
        return;
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_cache_pick_writing_slot(mbedtls_ssl_cache_context *cache,
                                       unsigned char const *session_id,
                                       size_t session_id_len,
                                       mbedtls_ssl_cache_entry **dst)
{
...
static inline int mbedtls_ssl_cache_set(void *data,
                          unsigned char const *session_id,
                          size_t session_id_len,
                          const mbedtls_ssl_session *session)
{
    ...
    session_serialized = (unsigned char*)mbedtls_calloc(1, session_serialized_len);
...
static inline int mbedtls_ssl_cache_remove(void *data,
                             unsigned char const *session_id,
                             size_t session_id_len)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
#if defined(MBEDTLS_HAVE_TIME)
static inline void mbedtls_ssl_cache_set_timeout(mbedtls_ssl_cache_context *cache, int timeout)
{
    if (timeout < 0) {
        timeout = 0;
    }
...
static inline void mbedtls_ssl_cache_set_max_entries(mbedtls_ssl_cache_context *cache, int max)
{
    if (max < 0) {
        max = 0;
    }
...
static inline void mbedtls_ssl_cache_free(mbedtls_ssl_cache_context *cache)
{
