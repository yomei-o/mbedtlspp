...
static inline  int ssl_conf_has_psk_or_cb(mbedtls_ssl_config const *conf)
{
    if (conf->f_psk != NULL) {
        return 1;
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_renegotiation_info_2(mbedtls_ssl_context *ssl,
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_supported_groups_ext(mbedtls_ssl_context *ssl,
                                          const unsigned char *buf,
                                          size_t len)
{
    ...
    if ((curves_tls_id = (uint16_t*)mbedtls_calloc(our_size,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_supported_point_formats(mbedtls_ssl_context *ssl,
                                             const unsigned char *buf,
                                             size_t len)
{
    size_t list_size;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_ecjpake_kkpp(mbedtls_ssl_context *ssl,
                                  const unsigned char *buf,
                                  size_t len)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_max_fragment_length_ext_2(mbedtls_ssl_context *ssl,
                                             const unsigned char *buf,
                                             size_t len)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_cid_ext_2(mbedtls_ssl_context *ssl,
                             const unsigned char *buf,
                             size_t len)
{
    size_t peer_cid_len;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_encrypt_then_mac_ext_2(mbedtls_ssl_context *ssl,
                                          const unsigned char *buf,
                                          size_t len)
{
    if (len != 0) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_extended_ms_ext_2(mbedtls_ssl_context *ssl,
                                     const unsigned char *buf,
                                     size_t len)
{
    if (len != 0) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_session_ticket_ext(mbedtls_ssl_context *ssl,
                                        unsigned char *buf,
                                        size_t len)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_use_srtp_ext(mbedtls_ssl_context *ssl,
                                  const unsigned char *buf,
                                  size_t len)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_check_key_curve(mbedtls_pk_context *pk,
                               uint16_t *curves_tls_id)
{
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_pick_cert(mbedtls_ssl_context *ssl,
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_ciphersuite_match(mbedtls_ssl_context *ssl, int suite_id,
                                 const mbedtls_ssl_ciphersuite_t **ciphersuite_info)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_client_hello(mbedtls_ssl_context *ssl)
{
    ...
    while (ext_len != 0) {
        ...
        switch (ext_id) {
            ...
            case MBEDTLS_TLS_EXT_RENEGOTIATION_INFO:
                ...
                ret = ssl_parse_renegotiation_info_2(ssl, ext + 4, ext_size);
                if (ret != 0) {
                    return ret;
                }
                break;
          ...
          MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED */
            ...
            case MBEDTLS_TLS_EXT_MAX_FRAGMENT_LENGTH:
                ...
                ret = ssl_parse_max_fragment_length_ext_2(ssl, ext + 4, ext_size);
                if (ret != 0) {
                    return ret;
                }
                break;
            ...
            case MBEDTLS_TLS_EXT_CID:
                ...
                ret = ssl_parse_cid_ext_2(ssl, ext + 4, ext_size);
                if (ret != 0) {
                    return ret;
                }
                break;
            ...
            case MBEDTLS_TLS_EXT_ENCRYPT_THEN_MAC:
                ...
                ret = ssl_parse_encrypt_then_mac_ext_2(ssl, ext + 4, ext_size);
                if (ret != 0) {
                    return ret;
                }
                break;
            ...
            case MBEDTLS_TLS_EXT_EXTENDED_MASTER_SECRET:
                ...
                ret = ssl_parse_extended_ms_ext_2(ssl, ext + 4, ext_size);
                if (ret != 0) {
                    return ret;
                }
                break;
...
#if defined(MBEDTLS_SSL_DTLS_CONNECTION_ID)
static inline  void ssl_write_cid_ext(mbedtls_ssl_context *ssl,
                              unsigned char *buf,
                              size_t *olen)
{
    unsigned char *p = buf;
...
static inline  void ssl_write_encrypt_then_mac_ext(mbedtls_ssl_context *ssl,
                                           unsigned char *buf,
                                           size_t *olen)
{
    unsigned char *p = buf;
...
#if defined(MBEDTLS_SSL_EXTENDED_MASTER_SECRET)
static inline  void ssl_write_extended_ms_ext(mbedtls_ssl_context *ssl,
                                      unsigned char *buf,
                                      size_t *olen)
{
    unsigned char *p = buf;
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
static inline  void ssl_write_session_ticket_ext(mbedtls_ssl_context *ssl,
                                         unsigned char *buf,
                                         size_t *olen)
{
    unsigned char *p = buf;
...
static inline  void ssl_write_renegotiation_ext(mbedtls_ssl_context *ssl,
                                        unsigned char *buf,
                                        size_t *olen)
{
    unsigned char *p = buf;
...
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
static inline  void ssl_write_max_fragment_length_ext(mbedtls_ssl_context *ssl,
                                              unsigned char *buf,
                                              size_t *olen)
{
    unsigned char *p = buf;
    ...
    *olen = 5;
    ...
    defined(MBEDTLS_KEY_EXCHANGE_ECDSA_CERT_REQ_ALLOWED_ENABLED) || \
    defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
static inline  void ssl_write_supported_point_formats_ext(mbedtls_ssl_context *ssl,
                                                  unsigned char *buf,
                                                  size_t *olen)
{
    unsigned char *p = buf;
...
#if defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
static inline  void ssl_write_ecjpake_kkpp_ext(mbedtls_ssl_context *ssl,
                                       unsigned char *buf,
                                       size_t *olen)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
static inline  void ssl_write_use_srtp_ext(mbedtls_ssl_context *ssl,
                                   unsigned char *buf,
                                   size_t *olen)
{
...
#if defined(MBEDTLS_SSL_DTLS_HELLO_VERIFY)
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_write_hello_verify_request(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
static inline  void ssl_handle_id_based_session_resumption(mbedtls_ssl_context *ssl)
{
    int ret;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_write_server_hello(mbedtls_ssl_context *ssl)
{
#if defined(MBEDTLS_HAVE_TIME)
    mbedtls_time_t t;
#endif
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_write_certificate_request(mbedtls_ssl_context *ssl)
{
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info =
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_resume_server_key_exchange(mbedtls_ssl_context *ssl,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_prepare_server_key_exchange(mbedtls_ssl_context *ssl,
                                           size_t *signature_len)
{
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info =
        ssl->handshake->ciphersuite_info;
 ...
 * machine. */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_write_server_key_exchange(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    if (mbedtls_ssl_ciphersuite_no_pfs(ciphersuite_info)) {
        ...
        if (mbedtls_ssl_ciphersuite_uses_ecdh(ciphersuite_info)) {
            ret = ssl_get_ecdh_params_from_cert_2(ssl);
            if (ret != 0) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_write_server_hello_done(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_client_dh_public(mbedtls_ssl_context *ssl, unsigned char **p,
...
#if defined(MBEDTLS_SSL_ASYNC_PRIVATE)
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_resume_decrypt_pms(mbedtls_ssl_context *ssl,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_decrypt_encrypted_pms(mbedtls_ssl_context *ssl,
                                     const unsigned char *p,
                                     const unsigned char *end,
                                     unsigned char *peer_pms,
                                     size_t *peer_pmslen,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_encrypted_pms(mbedtls_ssl_context *ssl,
                                   const unsigned char *p,
                                   const unsigned char *end,
                                   size_t pms_offset)
{
...
#if defined(MBEDTLS_KEY_EXCHANGE_SOME_PSK_ENABLED)
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_client_psk_identity(mbedtls_ssl_context *ssl, unsigned char **p,
                                         const unsigned char *end)
{
    int ret = 0;
    uint16_t n;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_client_key_exchange(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info;
    unsigned char *p, *end;
...
#if !defined(MBEDTLS_KEY_EXCHANGE_CERT_REQ_ALLOWED_ENABLED)
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_certificate_verify(mbedtls_ssl_context *ssl)
{
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info =
        ssl->handshake->ciphersuite_info;
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_write_new_session_ticket(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t tlen;
    uint32_t lifetime;
