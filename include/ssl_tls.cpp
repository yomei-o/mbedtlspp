...
static inline  int local_err_translation_9(psa_status_t status)
{
    return psa_status_to_mbedtls(status, psa_to_ssl_errors,
                                 ARRAY_LENGTH(psa_to_ssl_errors),
                                 psa_generic_status_to_mbedtls);
}
#define PSA_TO_MBEDTLS_ERR(status) local_err_translation_9(status)
...
static inline  const uint8_t jpake_server_id[] = { 's', 'e', 'r', 'v', 'e', 'r' };
static inline  const uint8_t jpake_client_id[] = { 'c', 'l', 'i', 'e', 'n', 't' };
...
static inline const uint16_t *mbedtls_ssl_get_supported_group_list(void)
{
...
#if defined(MBEDTLS_SSL_ALPN)
static inline int mbedtls_ssl_conf_alpn_protocols(mbedtls_ssl_config *conf,
...
static inline unsigned char mbedtls_ssl_sig_from_pk_alg(mbedtls_pk_sigalg_t type)
{
    switch (type) {
...
static inline mbedtls_pk_sigalg_t mbedtls_ssl_pk_sig_alg_from_sig(unsigned char sig)
{
    switch (sig) {
#if defined(PSA_WANT_KEY_TYPE_RSA_KEY_PAIR_BASIC)
                                         ...
                                         unsigned char *buf, int from)
{
    ...
    const char *sender;
    psa_status_t status;
    psa_hash_operation_t *hs_op = (psa_hash_operation_t *) ctx;
    psa_hash_operation_t cloned_op = PSA_HASH_OPERATION_INIT;
    size_t hash_size;
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
static inline  int mbedtls_ssl_tls12_export_keying_material(const mbedtls_ssl_context *ssl,
                                                    ...
                                                    const int use_context)
{
    ...
    prf_input = (unsigned char *) mbedtls_calloc(prf_input_len, sizeof(unsigned char));
    if (prf_input == NULL) {
        return MBEDTLS_ERR_SSL_ALLOC_FAILED;
    }
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
static inline  int mbedtls_ssl_tls13_export_keying_material(mbedtls_ssl_context *ssl,
                                                    const mbedtls_md_type_t hash_alg,
                                                    uint8_t *out,
                                                    const size_t key_len,
                                                    const char *label,
...
static inline int mbedtls_ssl_export_keying_material(mbedtls_ssl_context *ssl,
