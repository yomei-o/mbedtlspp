...
static inline  int err_from_posix(int posix_ret)
{
    switch (posix_ret) {
        case 0:
            return 0;
...
static inline  int threading_mutex_init_pthread(mbedtls_platform_mutex_t *mutex)
{
    int posix_ret = pthread_mutex_init(mutex, NULL);
    return err_from_posix(posix_ret);
}

static inline  void threading_mutex_destroy_pthread(mbedtls_platform_mutex_t *mutex)
{
    (void) pthread_mutex_destroy(mutex);
}

static inline  int threading_mutex_lock_pthread(mbedtls_platform_mutex_t *mutex)
{
    int posix_ret = pthread_mutex_lock(mutex);
    return err_from_posix(posix_ret);
}

static inline  int threading_mutex_unlock_pthread(mbedtls_platform_mutex_t *mutex)
{
...
static inline int mbedtls_condition_variable_init(
...
static inline void mbedtls_condition_variable_free(
    mbedtls_threading_condition_variable_t *cond)
{
    (void) pthread_cond_destroy(&cond->cond);
}

static inline int mbedtls_condition_variable_signal(
    mbedtls_threading_condition_variable_t *cond)
{
...
static inline int mbedtls_condition_variable_broadcast(
    mbedtls_threading_condition_variable_t *cond)
{
...
static inline int mbedtls_condition_variable_wait(
    mbedtls_threading_condition_variable_t *cond,
...
static inline  int threading_mutex_fail(mbedtls_platform_mutex_t *mutex)
{
    ((void) mutex);
    return PSA_ERROR_BAD_STATE;
}
static inline  void threading_mutex_dummy(mbedtls_platform_mutex_t *mutex)
{
    ((void) mutex);
    return;
}
...
static inline  int (*cond_init_ptr)(mbedtls_platform_condition_variable_t *) = NULL;
...
static inline  void (*cond_destroy_ptr)(mbedtls_platform_condition_variable_t *) = NULL;
...
static inline  int (*cond_signal_ptr)(mbedtls_platform_condition_variable_t *) = NULL;
...
static inline  int (*cond_broadcast_ptr)(mbedtls_platform_condition_variable_t *) = NULL;
...
static inline  int (*cond_wait_ptr)(mbedtls_platform_condition_variable_t *,
...
/*
 * Library auto-initialisation.
 *
 *   Register the C++ mutex / condition variable callbacks once at process
 *   start-up, before main() and before any thread exists. Registering also
 *   initialises the library global mutexes inside mbedtls_threading_set_alt().
 *
 *   This is what makes THREADING_ALT work without the application calling
 *   mbedtls_threading_set_alt() itself, which is easy to forget and fails in
 *   confusing ways -- every mbedtls context is created at run time, so this
 *   static initialiser always runs first.
 */
namespace {
struct mbedtls_threading_auto_init_t {
    mbedtls_threading_auto_init_t()
    {
        mbedtls_threading_set_alt(threading_mutex_init, threading_mutex_destroy,
                                  threading_mutex_lock, threading_mutex_unlock,
                                  threading_cond_init, threading_cond_destroy,
                                  threading_cond_signal, threading_cond_broadcast,
                                  threading_cond_wait);
    }
};
