...
#include "psa_crypto_core_common.hpp"

#if defined(MBEDTLS_PSA_CRYPTO_C)

#include "check_crypto_config.hpp"
#include "psa_crypto.hpp"
#include "psa_crypto_values.hpp"

#include "psa_crypto_cipher.hpp"
#include "psa_crypto_core.hpp"
#include "psa_crypto_invasive.hpp"
#include "psa_crypto_driver_wrappers.hpp"
#include "psa_crypto_driver_wrappers_no_static.hpp"
#include "psa_crypto_ecp.hpp"
#include "psa_crypto_ffdh.hpp"
#include "psa_crypto_hash.hpp"
#include "psa_crypto_mac.hpp"
#include "psa_crypto_rsa.hpp"
#include "psa_crypto_ecp.hpp"
#include "psa_crypto_slot_management.hpp"
/* Include internal declarations that are useful for implementing persistently
 * stored keys. */
#include "psa_crypto_storage.hpp"

#include "psa_crypto_random.hpp"
...
#include "mbedtls_private_chacha20.hpp"
...
static inline  psa_global_data_t global_data;

static inline  uint8_t psa_get_initialized(void)
{
    uint8_t initialized;
...
static inline  uint8_t psa_get_drivers_initialized(void)
{
    uint8_t initialized;
...
static inline int psa_is_ready_for_cipher(void)
{
...
static inline  int psa_is_dh_key_size_valid(size_t bits)
{
    switch (bits) {
...
static inline psa_status_t mbedtls_to_psa_error(int ret)
{
 ...
 */
static inline  void psa_wipe_tag_output_buffer(uint8_t *output_buffer, psa_status_t status,
...
static inline psa_status_t psa_validate_unstructured_key_bit_size(psa_key_type_t type,
                                                    size_t bits)
{
 ...
 */
static inline MBEDTLS_STATIC_TESTABLE psa_status_t psa_mac_key_can_do(
...
static inline psa_status_t psa_allocate_buffer_to_slot(psa_key_slot_t *slot,
...
static inline psa_status_t psa_copy_key_material_into_slot(psa_key_slot_t *slot,
                                             const uint8_t *data,
                                             size_t data_length)
{
...
static inline psa_status_t psa_import_key_into_slot(
 ...
 */
static inline  psa_algorithm_t psa_key_policy_algorithm_intersection(
    psa_key_type_t key_type,
...
static inline  int psa_key_algorithm_permits(psa_key_type_t key_type,
 ...
 */
static inline  psa_status_t psa_key_policy_permits(const psa_key_policy_t *policy,
                                           psa_key_type_t key_type,
                                           psa_algorithm_t alg)
{
 ...
 */
static inline  psa_status_t psa_restrict_key_policy(
    psa_key_type_t key_type,
 ...
 */
static inline  psa_status_t psa_get_and_lock_key_slot_with_policy(
 ...
 */
static inline  psa_status_t psa_get_and_lock_transparent_key_slot_with_policy(
    mbedtls_svc_key_id_t key,
    psa_key_slot_t **p_slot,
    psa_key_usage_t usage,
    psa_algorithm_t alg)
...
static inline psa_status_t psa_remove_key_data_from_memory(psa_key_slot_t *slot)
{
#if defined(MBEDTLS_PSA_STATIC_KEY_SLOTS)
    if (slot->key.bytes > 0) {
...
static inline psa_status_t psa_wipe_key_slot(psa_key_slot_t *slot)
{
 ...
 */
static inline psa_status_t psa_get_key_attributes(mbedtls_svc_key_id_t key,
...
static inline  psa_status_t psa_export_key_buffer_internal(const uint8_t *key_buffer,
...
static inline psa_status_t psa_export_key_internal(
    const psa_key_attributes_t *attributes,
...
static inline psa_status_t psa_export_key(mbedtls_svc_key_id_t key,
...
static inline psa_status_t psa_export_public_key_internal(
    const psa_key_attributes_t *attributes,
...
static inline psa_status_t psa_export_public_key(mbedtls_svc_key_id_t key,
                                   uint8_t *data_external,
                                   size_t data_size,
                                   size_t *data_length)
{
...
/****************************************************************/
    ...
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_PUBLIC_KEY))
static inline  psa_status_t psa_export_public_key_iop_abort_internal(psa_export_public_key_iop_t *operation)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
static inline uint32_t psa_export_public_key_iop_get_num_ops(psa_export_public_key_iop_t *operation)
{
#if defined(MBEDTLS_ECP_RESTARTABLE) && \
    (defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR_IMPORT) || \
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR_EXPORT) || \
...
static inline psa_status_t psa_export_public_key_iop_setup(psa_export_public_key_iop_t *operation,
                                             mbedtls_svc_key_id_t key)
{
#if defined(MBEDTLS_ECP_RESTARTABLE)  && \
    (defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR_IMPORT) || \
...
static inline psa_status_t psa_export_public_key_iop_complete(psa_export_public_key_iop_t *operation,
                                                uint8_t *data,
                                                size_t data_size,
                                                size_t *data_length)
{
...
static inline psa_status_t psa_export_public_key_iop_abort(psa_export_public_key_iop_t *operation)
{
#if defined(MBEDTLS_ECP_RESTARTABLE)  && \
    (defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR_IMPORT) || \
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR_EXPORT) || \
 ...
 */
static inline  psa_status_t psa_validate_key_policy(const psa_key_policy_t *policy)
{
 ...
 *
 */
static inline  psa_status_t psa_validate_key_attributes(const psa_key_attributes_t *attributes)
{
 ...
 */
static inline  psa_status_t psa_start_key_creation(
    const psa_key_attributes_t *attributes,
    psa_key_slot_t **p_slot)
{
    psa_status_t status;
 ...
 *         You must call psa_fail_key_creation() to wipe and free the slot.
 */
static inline  psa_status_t psa_finish_key_creation(
    psa_key_slot_t *slot,
    mbedtls_svc_key_id_t *key)
{
 ...
 * \param[in,out] slot  Pointer to the slot with key material.
 */
static inline  void psa_fail_key_creation(psa_key_slot_t *slot)
{
    if (slot == NULL) {
        return;
    }
 ...
 */
static inline  psa_status_t psa_validate_optional_attributes(
...
static inline psa_status_t psa_import_key(const psa_key_attributes_t *attributes,
...
static inline psa_status_t psa_copy_key(mbedtls_svc_key_id_t source_key,
...
static inline  int is_hash_supported(psa_algorithm_t alg)
{
    switch (alg) {
...
static inline psa_status_t psa_hash_setup(psa_hash_operation_t *operation,
                            psa_algorithm_t alg)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
static inline psa_status_t psa_hash_update(psa_hash_operation_t *operation,
...
static inline  psa_status_t psa_hash_finish_internal(psa_hash_operation_t *operation,
                                             uint8_t *hash,
                                             size_t hash_size,
                                             size_t *hash_length)
{
...
static inline psa_status_t psa_hash_finish(psa_hash_operation_t *operation,
...
static inline psa_status_t psa_hash_verify(psa_hash_operation_t *operation,
...
static inline psa_status_t psa_hash_compare(psa_algorithm_t alg,
                              const uint8_t *input_external, size_t input_length,
...
static inline psa_status_t psa_hash_clone(const psa_hash_operation_t *source_operation,
...
static inline psa_status_t psa_xof_abort(psa_xof_operation_t *operation)
{
    /* Aborting a non-active operation is allowed */
    if (operation->id == 0) {
        return PSA_SUCCESS;
...
static inline psa_status_t psa_xof_setup(psa_xof_operation_t *operation,
                           psa_algorithm_t alg)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
static inline psa_status_t psa_xof_set_context(psa_xof_operation_t *operation,
...
static inline psa_status_t psa_xof_update(psa_xof_operation_t *operation,
                            const uint8_t *input_external,
                            size_t input_length)
{
    if (operation->id == 0) {
...
static inline psa_status_t psa_xof_output(psa_xof_operation_t *operation,
...
static inline psa_status_t psa_mac_abort(psa_mac_operation_t *operation)
{
    /* Aborting a non-active operation is allowed */
    if (operation->id == 0) {
        return PSA_SUCCESS;
...
static inline  psa_status_t psa_mac_finalize_alg_and_key_validation(
    psa_algorithm_t alg,
    const psa_key_attributes_t *attributes,
    uint8_t *mac_size)
{
...
static inline  psa_status_t psa_mac_setup(psa_mac_operation_t *operation,
                                  mbedtls_svc_key_id_t key,
                                  psa_algorithm_t alg,
                                  int is_sign)
{
...
static inline psa_status_t psa_mac_sign_setup(psa_mac_operation_t *operation,
                                mbedtls_svc_key_id_t key,
                                psa_algorithm_t alg)
{
...
static inline psa_status_t psa_mac_verify_setup(psa_mac_operation_t *operation,
                                  mbedtls_svc_key_id_t key,
                                  psa_algorithm_t alg)
{
...
static inline psa_status_t psa_mac_update(psa_mac_operation_t *operation,
                            const uint8_t *input_external,
                            size_t input_length)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
static inline psa_status_t psa_mac_sign_finish(psa_mac_operation_t *operation,
...
static inline psa_status_t psa_mac_verify_finish(psa_mac_operation_t *operation,
...
static inline  psa_status_t psa_mac_compute_internal(mbedtls_svc_key_id_t key,
                                             psa_algorithm_t alg,
                                             const uint8_t *input,
                                             size_t input_length,
                                             uint8_t *mac,
...
static inline psa_status_t psa_mac_compute(mbedtls_svc_key_id_t key,
                             psa_algorithm_t alg,
                             const uint8_t *input_external,
                             size_t input_length,
                             uint8_t *mac_external,
...
static inline psa_status_t psa_mac_verify(mbedtls_svc_key_id_t key,
                            psa_algorithm_t alg,
                            const uint8_t *input_external,
                            size_t input_length,
                            const uint8_t *mac_external,
...
static inline  psa_status_t psa_sign_verify_check_alg(int input_is_message,
                                              psa_algorithm_t alg)
{
    if (input_is_message) {
...
static inline  psa_status_t psa_sign_internal(mbedtls_svc_key_id_t key,
...
static inline  psa_status_t psa_verify_internal(mbedtls_svc_key_id_t key,
                                        int input_is_message,
                                        psa_algorithm_t alg,
                                        const uint8_t *input,
                                        size_t input_length,
...
static inline psa_status_t psa_sign_message_builtin(
    const psa_key_attributes_t *attributes,
    const uint8_t *key_buffer,
    size_t key_buffer_size,
    psa_algorithm_t alg,
...
static inline psa_status_t psa_sign_message(mbedtls_svc_key_id_t key,
                              psa_algorithm_t alg,
                              const uint8_t *input_external,
                              size_t input_length,
...
static inline psa_status_t psa_verify_message_builtin(
    const psa_key_attributes_t *attributes,
    const uint8_t *key_buffer,
    size_t key_buffer_size,
    psa_algorithm_t alg,
...
static inline psa_status_t psa_verify_message(mbedtls_svc_key_id_t key,
                                psa_algorithm_t alg,
                                const uint8_t *input_external,
                                size_t input_length,
...
static inline psa_status_t psa_sign_hash_builtin(
    const psa_key_attributes_t *attributes,
    const uint8_t *key_buffer, size_t key_buffer_size,
...
static inline psa_status_t psa_sign_hash(mbedtls_svc_key_id_t key,
                           psa_algorithm_t alg,
                           const uint8_t *hash_external,
                           size_t hash_length,
                           uint8_t *signature_external,
...
static inline psa_status_t psa_verify_hash_builtin(
    const psa_key_attributes_t *attributes,
    const uint8_t *key_buffer, size_t key_buffer_size,
    psa_algorithm_t alg, const uint8_t *hash, size_t hash_length,
...
static inline psa_status_t psa_verify_hash(mbedtls_svc_key_id_t key,
                             psa_algorithm_t alg,
                             const uint8_t *hash_external,
                             size_t hash_length,
                             const uint8_t *signature_external,
...
static inline psa_status_t psa_asymmetric_encrypt(mbedtls_svc_key_id_t key,
                                    psa_algorithm_t alg,
                                    const uint8_t *input_external,
                                    size_t input_length,
...
static inline psa_status_t psa_asymmetric_decrypt(mbedtls_svc_key_id_t key,
                                    psa_algorithm_t alg,
                                    const uint8_t *input_external,
                                    size_t input_length,
                                    const uint8_t *salt_external,
...
static inline  uint32_t psa_interruptible_max_ops = PSA_INTERRUPTIBLE_MAX_OPS_UNLIMITED;

static inline void psa_interruptible_set_max_ops(uint32_t max_ops)
{
    psa_interruptible_max_ops = max_ops;
}

static inline uint32_t psa_interruptible_get_max_ops(void)
{
    return psa_interruptible_max_ops;
}

static inline uint32_t psa_sign_hash_get_num_ops(
    const psa_sign_hash_interruptible_operation_t *operation)
{
    return operation->num_ops;
}

static inline uint32_t psa_verify_hash_get_num_ops(
    const psa_verify_hash_interruptible_operation_t *operation)
{
    return operation->num_ops;
}

static inline  psa_status_t psa_sign_hash_abort_internal(
...
static inline psa_status_t psa_sign_hash_start(
    psa_sign_hash_interruptible_operation_t *operation,
...
static inline psa_status_t psa_sign_hash_complete(
    psa_sign_hash_interruptible_operation_t *operation,
...
static inline psa_status_t psa_sign_hash_abort(
    psa_sign_hash_interruptible_operation_t *operation)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
static inline  psa_status_t psa_verify_hash_abort_internal(
...
static inline psa_status_t psa_verify_hash_start(
    psa_verify_hash_interruptible_operation_t *operation,
    mbedtls_svc_key_id_t key, psa_algorithm_t alg,
    const uint8_t *hash_external, size_t hash_length,
...
static inline psa_status_t psa_verify_hash_complete(
    psa_verify_hash_interruptible_operation_t *operation)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
static inline psa_status_t psa_verify_hash_abort(
    psa_verify_hash_interruptible_operation_t *operation)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
static inline void mbedtls_psa_interruptible_set_max_ops(uint32_t max_ops)
{
...
static inline uint32_t mbedtls_psa_sign_hash_get_num_ops(
...
static inline uint32_t mbedtls_psa_verify_hash_get_num_ops(
...
static inline psa_status_t mbedtls_psa_sign_hash_start(
...
static inline psa_status_t mbedtls_psa_sign_hash_complete(
    mbedtls_psa_sign_hash_interruptible_operation_t *operation,
...
static inline psa_status_t mbedtls_psa_sign_hash_abort(
    mbedtls_psa_sign_hash_interruptible_operation_t *operation)
{
...
static inline psa_status_t mbedtls_psa_verify_hash_start(
...
static inline psa_status_t mbedtls_psa_verify_hash_complete(
    mbedtls_psa_verify_hash_interruptible_operation_t *operation)
{
...
static inline psa_status_t mbedtls_psa_verify_hash_abort(
    mbedtls_psa_verify_hash_interruptible_operation_t *operation)
{
...
static inline  psa_status_t psa_generate_random_internal(uint8_t *output,
                                                 size_t output_size)
{
...
static inline  psa_status_t psa_cipher_setup(psa_cipher_operation_t *operation,
                                     mbedtls_svc_key_id_t key,
                                     psa_algorithm_t alg,
...
static inline psa_status_t psa_cipher_encrypt_setup(psa_cipher_operation_t *operation,
                                      mbedtls_svc_key_id_t key,
                                      psa_algorithm_t alg)
{
...
static inline psa_status_t psa_cipher_decrypt_setup(psa_cipher_operation_t *operation,
                                      mbedtls_svc_key_id_t key,
                                      psa_algorithm_t alg)
{
...
static inline psa_status_t psa_cipher_generate_iv(psa_cipher_operation_t *operation,
                                    uint8_t *iv_external,
                                    size_t iv_size,
                                    size_t *iv_length)
{
...
static inline psa_status_t psa_cipher_set_iv(psa_cipher_operation_t *operation,
...
static inline psa_status_t psa_cipher_update(psa_cipher_operation_t *operation,
                               const uint8_t *input_external,
                               size_t input_length,
                               uint8_t *output_external,
                               size_t output_size,
...
static inline psa_status_t psa_cipher_finish(psa_cipher_operation_t *operation,
                               uint8_t *output_external,
                               size_t output_size,
                               size_t *output_length)
{
...
static inline psa_status_t psa_cipher_abort(psa_cipher_operation_t *operation)
{
    if (operation->id == 0) {
        /* The object has (apparently) been initialized but it is not (yet)
         * in use. It's ok to call abort on such an object, and there's
...
static inline psa_status_t psa_cipher_encrypt(mbedtls_svc_key_id_t key,
                                psa_algorithm_t alg,
                                const uint8_t *input_external,
                                size_t input_length,
                                uint8_t *output_external,
...
static inline psa_status_t psa_cipher_decrypt(mbedtls_svc_key_id_t key,
                                psa_algorithm_t alg,
                                const uint8_t *input_external,
                                size_t input_length,
                                uint8_t *output_external,
...
static inline  psa_algorithm_t psa_aead_get_base_algorithm(psa_algorithm_t alg)
{
    return PSA_ALG_AEAD_WITH_DEFAULT_LENGTH_TAG(alg);
}

/* Helper function to perform common nonce length checks. */
static inline  psa_status_t psa_aead_check_nonce_length(psa_algorithm_t alg,
                                                size_t nonce_length)
{
...
static inline  psa_status_t psa_aead_check_algorithm(psa_algorithm_t alg)
{
...
static inline psa_status_t psa_aead_encrypt(mbedtls_svc_key_id_t key,
                              psa_algorithm_t alg,
...
static inline psa_status_t psa_aead_decrypt(mbedtls_svc_key_id_t key,
                              psa_algorithm_t alg,
                              const uint8_t *nonce_external,
                              size_t nonce_length,
                              const uint8_t *additional_data_external,
...
static inline  psa_status_t psa_validate_tag_length(psa_algorithm_t alg)
{
...
static inline  psa_status_t psa_aead_setup(psa_aead_operation_t *operation,
                                   int is_encrypt,
                                   mbedtls_svc_key_id_t key,
                                   psa_algorithm_t alg)
{
...
static inline psa_status_t psa_aead_encrypt_setup(psa_aead_operation_t *operation,
                                    mbedtls_svc_key_id_t key,
                                    psa_algorithm_t alg)
{
...
static inline psa_status_t psa_aead_decrypt_setup(psa_aead_operation_t *operation,
                                    mbedtls_svc_key_id_t key,
                                    psa_algorithm_t alg)
{
...
static inline  psa_status_t psa_aead_set_nonce_internal(psa_aead_operation_t *operation,
                                                const uint8_t *nonce,
                                                size_t nonce_length)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
static inline psa_status_t psa_aead_generate_nonce(psa_aead_operation_t *operation,
...
/* Set the nonce for a multipart authenticated encryption or decryption
   operation.*/
static inline psa_status_t psa_aead_set_nonce(psa_aead_operation_t *operation,
                                const uint8_t *nonce_external,
                                size_t nonce_length)
{
    psa_status_t status;
...
static inline psa_status_t psa_aead_set_lengths(psa_aead_operation_t *operation,
                                  size_t ad_length,
                                  size_t plaintext_length)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
static inline psa_status_t psa_aead_update_ad(psa_aead_operation_t *operation,
                                const uint8_t *input_external,
                                size_t input_length)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
/* Encrypt or decrypt a message fragment in an active multipart AEAD
   operation.*/
static inline psa_status_t psa_aead_update(psa_aead_operation_t *operation,
                             const uint8_t *input_external,
                             size_t input_length,
                             uint8_t *output_external,
                             size_t output_size,
...
static inline  psa_status_t psa_aead_final_checks(const psa_aead_operation_t *operation)
{
...
static inline psa_status_t psa_aead_finish(psa_aead_operation_t *operation,
                             uint8_t *ciphertext_external,
                             size_t ciphertext_size,
                             size_t *ciphertext_length,
...
/* Finish authenticating and decrypting a message in a multipart AEAD
   operation.*/
static inline psa_status_t psa_aead_verify(psa_aead_operation_t *operation,
                             uint8_t *plaintext_external,
                             size_t plaintext_size,
                             size_t *plaintext_length,
...
static inline psa_status_t psa_aead_abort(psa_aead_operation_t *operation)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
static inline  psa_algorithm_t psa_key_derivation_get_kdf_alg(
...
static inline psa_status_t psa_key_derivation_abort(psa_key_derivation_operation_t *operation)
{
    psa_status_t status = PSA_SUCCESS;
...
static inline psa_status_t psa_key_derivation_get_capacity(const psa_key_derivation_operation_t *operation,
                                             size_t *capacity)
{
    if (operation->alg == 0) {
...
static inline psa_status_t psa_key_derivation_set_capacity(psa_key_derivation_operation_t *operation,
                                             size_t capacity)
{
    if (operation->alg == 0) {
        return PSA_ERROR_BAD_STATE;
...
static inline  psa_status_t psa_key_derivation_hkdf_read(psa_hkdf_key_derivation_t *hkdf,
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PRF) || \
    defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS)
static inline  psa_status_t psa_key_derivation_tls12_prf_generate_next_block(
...
static inline  psa_status_t psa_key_derivation_tls12_prf_read(
    psa_tls12_prf_key_derivation_t *tls12_prf,
    psa_algorithm_t alg,
    uint8_t *output,
    size_t output_length)
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_ECJPAKE_TO_PMS)
static inline  psa_status_t psa_key_derivation_tls12_ecjpake_to_pms_read(
...
#if defined(PSA_HAVE_SOFT_PBKDF2)
static inline  psa_status_t psa_key_derivation_pbkdf2_generate_block(
...
static inline  psa_status_t psa_key_derivation_pbkdf2_read(
    psa_pbkdf2_key_derivation_t *pbkdf2,
    psa_algorithm_t kdf_alg,
    uint8_t *output,
    size_t output_length)
...
static inline psa_status_t psa_key_derivation_output_bytes(
...
static inline  psa_status_t psa_generate_derived_ecc_key_weierstrass_helper(
    psa_key_slot_t *slot,
    size_t bits,
    psa_key_derivation_operation_t *operation,
    uint8_t **data
...
static inline  psa_status_t psa_generate_derived_ecc_key_montgomery_helper(
    size_t bits,
    psa_key_derivation_operation_t *operation,
    uint8_t **data
    )
...
static inline  psa_status_t psa_generate_derived_key_internal(
    psa_key_slot_t *slot,
    size_t bits,
    psa_key_derivation_operation_t *operation)
{
...
static inline  const psa_custom_key_parameters_t default_custom_production =
    PSA_CUSTOM_KEY_PARAMETERS_INIT;

static inline int psa_custom_key_parameters_are_default(
...
static inline psa_status_t psa_key_derivation_output_key_custom(
    const psa_key_attributes_t *attributes,
    psa_key_derivation_operation_t *operation,
    const psa_custom_key_parameters_t *custom,
...
static inline psa_status_t psa_key_derivation_output_key(
    const psa_key_attributes_t *attributes,
    psa_key_derivation_operation_t *operation,
    mbedtls_svc_key_id_t *key)
{
...
static inline psa_status_t psa_key_derivation_verify_bytes(
    psa_key_derivation_operation_t *operation,
...
static inline psa_status_t psa_key_derivation_verify_key(
    psa_key_derivation_operation_t *operation,
...
static inline  int is_kdf_alg_supported(psa_algorithm_t kdf_alg)
{
#if defined(MBEDTLS_PSA_BUILTIN_ALG_HKDF)
...
static inline  psa_status_t psa_hash_try_support(psa_algorithm_t alg)
{
...
static inline  psa_status_t psa_key_derivation_set_maximum_capacity(
    psa_key_derivation_operation_t *operation,
    psa_algorithm_t kdf_alg)
{
...
static inline  psa_status_t psa_key_derivation_setup_kdf(
    psa_key_derivation_operation_t *operation,
    psa_algorithm_t kdf_alg)
{
...
static inline  psa_status_t psa_key_agreement_try_support(psa_algorithm_t alg)
{
...
static inline  int psa_key_derivation_allows_free_form_secret_input(
    psa_algorithm_t kdf_alg)
{
#if defined(PSA_WANT_ALG_TLS12_ECJPAKE_TO_PMS)
    if (kdf_alg == PSA_ALG_TLS12_ECJPAKE_TO_PMS) {
...
static inline psa_status_t psa_key_derivation_setup(psa_key_derivation_operation_t *operation,
                                      psa_algorithm_t alg)
{
#if defined(AT_LEAST_ONE_BUILTIN_KDF)
    psa_status_t status;
...
#if defined(BUILTIN_ALG_ANY_HKDF)
static inline  psa_status_t psa_hkdf_input(psa_hkdf_key_derivation_t *hkdf,
                                   psa_algorithm_t kdf_alg,
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PRF) || \
    defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS)
static inline  psa_status_t psa_tls12_prf_set_seed(psa_tls12_prf_key_derivation_t *prf,
                                           const uint8_t *data,
                                           size_t data_length)
{
...
static inline  psa_status_t psa_tls12_prf_set_key(psa_tls12_prf_key_derivation_t *prf,
                                          const uint8_t *data,
                                          size_t data_length)
{
...
static inline  psa_status_t psa_tls12_prf_set_label(psa_tls12_prf_key_derivation_t *prf,
                                            const uint8_t *data,
                                            size_t data_length)
{
...
static inline  psa_status_t psa_tls12_prf_input(psa_tls12_prf_key_derivation_t *prf,
                                        psa_key_derivation_step_t step,
                                        const uint8_t *data,
                                        size_t data_length)
{
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS)
static inline  psa_status_t psa_tls12_prf_psk_to_ms_set_key(
...
static inline  psa_status_t psa_tls12_prf_psk_to_ms_set_other_key(
    psa_tls12_prf_key_derivation_t *prf,
    const uint8_t *data,
    size_t data_length)
{
...
static inline  psa_status_t psa_tls12_prf_psk_to_ms_input(
    psa_tls12_prf_key_derivation_t *prf,
    psa_key_derivation_step_t step,
    const uint8_t *data,
    size_t data_length)
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_ECJPAKE_TO_PMS)
static inline  psa_status_t psa_tls12_ecjpake_to_pms_input(
    psa_tls12_ecjpake_to_pms_t *ecjpake,
    psa_key_derivation_step_t step,
    const uint8_t *data,
    size_t data_length)
...
#if defined(PSA_HAVE_SOFT_PBKDF2)
static inline  psa_status_t psa_pbkdf2_set_input_cost(
    psa_pbkdf2_key_derivation_t *pbkdf2,
    psa_key_derivation_step_t step,
    uint64_t data)
{
...
static inline  psa_status_t psa_pbkdf2_set_salt(psa_pbkdf2_key_derivation_t *pbkdf2,
                                        const uint8_t *data,
                                        size_t data_length)
{
    ...
    if (data_length == 0) {
        ...
        next_salt = (uint8_t *) mbedtls_calloc(1, data_length + pbkdf2->salt_length);
        if (next_salt == NULL) {
            return PSA_ERROR_INSUFFICIENT_MEMORY;
        }
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_PBKDF2_HMAC)
static inline  psa_status_t psa_pbkdf2_hmac_set_password(psa_algorithm_t hash_alg,
                                                 const uint8_t *input,
                                                 size_t input_len,
                                                 uint8_t *output,
                                                 size_t *output_len)
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_PBKDF2_AES_CMAC_PRF_128)
static inline  psa_status_t psa_pbkdf2_cmac_set_password(const uint8_t *input,
                                                 size_t input_len,
                                                 uint8_t *output,
                                                 size_t *output_len)
{
...
static inline  psa_status_t psa_pbkdf2_set_password(psa_pbkdf2_key_derivation_t *pbkdf2,
                                            psa_algorithm_t kdf_alg,
                                            const uint8_t *data,
                                            size_t data_length)
{
...
static inline  psa_status_t psa_pbkdf2_input(psa_pbkdf2_key_derivation_t *pbkdf2,
                                     psa_algorithm_t kdf_alg,
                                     psa_key_derivation_step_t step,
                                     const uint8_t *data,
                                     size_t data_length)
 ...
 */
static inline  int psa_key_derivation_check_input_type(
    psa_key_derivation_step_t step,
    psa_key_type_t key_type)
{
    switch (step) {
...
static inline  psa_status_t psa_key_derivation_input_internal(
    psa_key_derivation_operation_t *operation,
    psa_key_derivation_step_t step,
    psa_key_type_t key_type,
    const uint8_t *data,
...
static inline  psa_status_t psa_key_derivation_input_integer_internal(
    psa_key_derivation_operation_t *operation,
    psa_key_derivation_step_t step,
    uint64_t value)
{
...
static inline psa_status_t psa_key_derivation_input_bytes(
    psa_key_derivation_operation_t *operation,
    psa_key_derivation_step_t step,
    const uint8_t *data_external,
    size_t data_length)
...
static inline psa_status_t psa_key_derivation_input_integer(
    psa_key_derivation_operation_t *operation,
    psa_key_derivation_step_t step,
    uint64_t value)
{
...
static inline psa_status_t psa_key_derivation_input_key(
    psa_key_derivation_operation_t *operation,
    psa_key_derivation_step_t step,
    mbedtls_svc_key_id_t key)
{
...
static inline psa_status_t psa_key_agreement_raw_builtin(const psa_key_attributes_t *attributes,
                                           const uint8_t *key_buffer,
                                           size_t key_buffer_size,
                                           psa_algorithm_t alg,
 ...
 */
static inline  psa_status_t psa_key_agreement_raw_internal(psa_algorithm_t alg,
 ...
 */
static inline  psa_status_t psa_key_agreement_internal(psa_key_derivation_operation_t *operation,
                                               psa_key_derivation_step_t step,
                                               psa_key_slot_t *private_key,
                                               const uint8_t *peer_key,
                                               size_t peer_key_length)
...
static inline psa_status_t psa_key_derivation_key_agreement(psa_key_derivation_operation_t *operation,
                                              psa_key_derivation_step_t step,
...
static inline psa_status_t psa_raw_key_agreement(psa_algorithm_t alg,
                                   mbedtls_svc_key_id_t private_key,
                                   const uint8_t *peer_key_external,
                                   size_t peer_key_length,
                                   uint8_t *output_external,
...
static inline  psa_status_t validate_key_agreement_params(const psa_key_attributes_t *attributes,
                                                  psa_algorithm_t alg)
{
    psa_key_type_t key_type;
...
static inline psa_status_t psa_key_agreement(mbedtls_svc_key_id_t private_key,
                               const uint8_t *peer_key,
                               size_t peer_key_length,
                               psa_algorithm_t alg,
                               const psa_key_attributes_t *attributes,
...
static inline  psa_status_t psa_key_agreement_iop_abort_internal(psa_key_agreement_iop_t *operation)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
static inline uint32_t psa_key_agreement_iop_get_num_ops(
...
static inline psa_status_t psa_key_agreement_iop_setup(
    psa_key_agreement_iop_t *operation,
    mbedtls_svc_key_id_t private_key,
    const uint8_t *peer_key,
    size_t peer_key_length,
...
static inline psa_status_t psa_key_agreement_iop_complete(
    psa_key_agreement_iop_t *operation,
    mbedtls_svc_key_id_t *key)
{
#if defined(MBEDTLS_ECP_RESTARTABLE) && \
...
static inline psa_status_t psa_key_agreement_iop_abort(
    psa_key_agreement_iop_t *operation)
{
#if defined(MBEDTLS_ECP_RESTARTABLE) && \
    defined(MBEDTLS_PSA_BUILTIN_ALG_ECDH)
 ...
 */
static inline  void mbedtls_psa_random_init(mbedtls_psa_random_context_t *rng)
{
#if defined(MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG)
 ...
 *  this function if mutexes are enabled.
 */
static inline  void mbedtls_psa_random_free(mbedtls_psa_random_context_t *rng)
{
#if defined(MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG)
    memset(rng, 0, sizeof(*rng));
#else /* MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG */
 ...
 */
static inline  psa_status_t mbedtls_psa_random_seed(mbedtls_psa_random_context_t *rng)
{
#if defined(MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG)
...
static inline psa_status_t psa_random_reseed(const uint8_t *perso, size_t perso_size)
{
    GUARD_MODULE_INITIALIZED;
#if defined(MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG)
    (void) perso;
...
static inline psa_status_t psa_random_deplete(void)
{
    GUARD_MODULE_INITIALIZED;
#if defined(MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG)
    return PSA_ERROR_NOT_SUPPORTED;
...
static inline psa_status_t psa_random_set_prediction_resistance(unsigned enabled)
{
    GUARD_MODULE_INITIALIZED;
...
static inline psa_status_t psa_generate_random(uint8_t *output_external,
                                 size_t output_size)
{
    psa_status_t status;
 ...
 */
static inline  psa_status_t psa_validate_key_type_and_size_for_key_generation(
...
static inline psa_status_t psa_generate_key_internal(
    const psa_key_attributes_t *attributes,
    const psa_custom_key_parameters_t *custom,
    const uint8_t *custom_data,
    size_t custom_data_length,
...
static inline psa_status_t psa_generate_key_custom(const psa_key_attributes_t *attributes,
                                     const psa_custom_key_parameters_t *custom,
                                     const uint8_t *custom_data,
                                     size_t custom_data_length,
                                     mbedtls_svc_key_id_t *key)
...
static inline psa_status_t psa_generate_key(const psa_key_attributes_t *attributes,
                              mbedtls_svc_key_id_t *key)
{
...
#if defined(MBEDTLS_ECP_RESTARTABLE) && \
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR_GENERATE)
static inline  psa_status_t psa_generate_key_iop_abort_internal(
...
static inline uint32_t psa_generate_key_iop_get_num_ops(
    psa_generate_key_iop_t *operation)
{
    return operation->num_ops;
}

static inline psa_status_t psa_generate_key_iop_setup(
    psa_generate_key_iop_t *operation,
    const psa_key_attributes_t *attributes)
{
#if defined(MBEDTLS_ECP_RESTARTABLE) && \
...
static inline psa_status_t psa_generate_key_iop_complete(
    psa_generate_key_iop_t *operation,
    mbedtls_svc_key_id_t *key)
{
#if defined(MBEDTLS_ECP_RESTARTABLE) && \
...
static inline psa_status_t psa_generate_key_iop_abort(
    psa_generate_key_iop_t *operation)
{
#if defined(MBEDTLS_ECP_RESTARTABLE) && \
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR_GENERATE)
...
#if !defined(MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG)
static inline psa_status_t mbedtls_psa_crypto_configure_entropy_sources(
...
static inline  psa_status_t mbedtls_psa_crypto_init_subsystem(mbedtls_psa_crypto_subsystem subsystem)
{
    psa_status_t status = PSA_SUCCESS;
...
static inline psa_status_t psa_crypto_driver_pake_get_password_len(
...
static inline psa_status_t psa_crypto_driver_pake_get_password(
    const psa_crypto_driver_pake_inputs_t *inputs,
...
static inline psa_status_t psa_crypto_driver_pake_get_user_len(
    const psa_crypto_driver_pake_inputs_t *inputs,
    size_t *user_len)
{
...
static inline psa_status_t psa_crypto_driver_pake_get_user(
    const psa_crypto_driver_pake_inputs_t *inputs,
...
static inline psa_status_t psa_crypto_driver_pake_get_peer_len(
    const psa_crypto_driver_pake_inputs_t *inputs,
    size_t *peer_len)
{
...
static inline psa_status_t psa_crypto_driver_pake_get_peer(
    const psa_crypto_driver_pake_inputs_t *inputs,
...
static inline psa_status_t psa_crypto_driver_pake_get_cipher_suite(
    const psa_crypto_driver_pake_inputs_t *inputs,
...
static inline  psa_status_t psa_pake_set_password_key(
...
static inline psa_status_t psa_pake_setup(
    psa_pake_operation_t *operation,
...
static inline psa_status_t psa_pake_set_user(
    psa_pake_operation_t *operation,
...
static inline psa_status_t psa_pake_set_peer(
    psa_pake_operation_t *operation,
...
static inline psa_status_t psa_pake_set_role(
    psa_pake_operation_t *operation,
...
static inline psa_status_t psa_pake_set_context(
    psa_pake_operation_t *operation,
...
#if defined(PSA_WANT_ALG_JPAKE)
static inline  psa_crypto_driver_pake_step_t convert_jpake_computation_stage_to_driver_step(
...
static inline  psa_status_t psa_pake_complete_inputs(
    psa_pake_operation_t *operation)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
#if defined(PSA_WANT_ALG_JPAKE)
static inline  psa_status_t psa_jpake_prologue(
    psa_pake_operation_t *operation,
...
static inline  psa_status_t psa_jpake_epilogue(
    psa_pake_operation_t *operation,
    psa_jpake_io_mode_t io_mode)
{
    psa_jpake_computation_stage_t *stage =
...
static inline psa_status_t psa_pake_output(
    psa_pake_operation_t *operation,
    psa_pake_step_t step,
    uint8_t *output_external,
    size_t output_size,
...
static inline psa_status_t psa_pake_input(
    psa_pake_operation_t *operation,
    psa_pake_step_t step,
    const uint8_t *input_external,
    size_t input_length)
...
static inline psa_status_t psa_pake_get_shared_key(psa_pake_operation_t *operation,
                                     const psa_key_attributes_t *attributes,
                                     mbedtls_svc_key_id_t *key)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
static inline psa_status_t psa_pake_abort(
    psa_pake_operation_t *operation)
{
    psa_status_t status = PSA_SUCCESS;
...
static inline void (*psa_input_pre_copy_hook)(const uint8_t *input, size_t input_len) = NULL;
static inline void (*psa_input_post_copy_hook)(const uint8_t *input, size_t input_len) = NULL;
static inline void (*psa_output_pre_copy_hook)(const uint8_t *output, size_t output_len) = NULL;
static inline void (*psa_output_post_copy_hook)(const uint8_t *output, size_t output_len) = NULL;
#endif
...
static inline psa_status_t psa_crypto_copy_input(const uint8_t *input, size_t input_len,
 ...
 */
MBEDTLS_STATIC_TESTABLE
static inline psa_status_t psa_crypto_copy_output(const uint8_t *output_copy, size_t output_copy_len,
...
static inline psa_status_t psa_crypto_local_input_alloc(const uint8_t *input, size_t input_len,
...
static inline void psa_crypto_local_input_free(psa_crypto_local_input_t *local_input)
{
    mbedtls_zeroize_and_free(local_input->buffer, local_input->length);
    local_input->buffer = NULL;
    local_input->length = 0;
...
static inline psa_status_t psa_crypto_local_output_alloc(uint8_t *output, size_t output_len,
...
static inline psa_status_t psa_crypto_local_output_free(psa_crypto_local_output_t *local_output)
{
    psa_status_t status;
