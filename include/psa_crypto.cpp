...
#include "mbedtls_des.hpp"
#include "mbedtls_ecdh.hpp"
#include "mbedtls_ecp.hpp"
...
static inline int psa_can_do_cipher(psa_key_type_t key_type, psa_algorithm_t cipher_alg)
{
    (void) key_type;
    (void) cipher_alg;
    return psa_get_drivers_initialized();
...
#if defined(MBEDTLS_PSA_CRYPTO_SE_C)
static inline psa_status_t psa_get_key_slot_number(
    const psa_key_attributes_t *attributes,
 ...
 *
 */
static inline  psa_status_t psa_validate_key_attributes(
    const psa_key_attributes_t *attributes,
...
#if defined(MBEDTLS_PSA_CRYPTO_SE_C)
static inline psa_status_t mbedtls_psa_register_se_key(
    const psa_key_attributes_t *attributes)
{
    psa_status_t status;
    psa_key_slot_t *slot = NULL;
...
static inline  void psa_des_set_key_parity(uint8_t *data, size_t data_size)
{
    if (data_size >= 8) {
...
static inline psa_status_t psa_key_derivation_output_key_ext(
    const psa_key_attributes_t *attributes,
    psa_key_derivation_operation_t *operation,
...
#if defined(MBEDTLS_PSA_INJECT_ENTROPY)
static inline psa_status_t mbedtls_psa_inject_entropy(const uint8_t *seed,
                                        size_t seed_size)
{
...
static inline psa_status_t psa_generate_key_ext(const psa_key_attributes_t *attributes,
                                  const psa_key_production_parameters_t *params,
                                  size_t params_data_length,
                                  mbedtls_svc_key_id_t *key)
{
 ...
 * fails.
 */
static inline  psa_status_t psa_crypto_recover_transaction(
...
static inline psa_status_t psa_pake_get_implicit_key(
    psa_pake_operation_t *operation,
