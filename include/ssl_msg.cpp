...
#include "constant_time_internal.hpp" // for internal mbedtls_ct_xxx functions
...
static inline  int local_err_translation_7(psa_status_t status)
{
    return psa_status_to_mbedtls(status, psa_to_ssl_errors,
                                 ARRAY_LENGTH(psa_to_ssl_errors),
                                 psa_generic_status_to_mbedtls);
}
#define PSA_TO_MBEDTLS_ERR(status) local_err_translation_7(status)
...
static inline  uint32_t ssl_get_hs_total_len(mbedtls_ssl_context const *ssl);
...
static inline  int ssl_parse_record_header(mbedtls_ssl_context const *ssl,
                                   unsigned char *buf,
                                   size_t len,
                                   mbedtls_record *rec);
...
static inline  void ssl_buffering_free_slot(mbedtls_ssl_context *ssl,
                                    uint8_t slot);
static inline  void ssl_buffering_shift_slots(mbedtls_ssl_context *ssl, unsigned shift);
static inline  void ssl_free_buffered_record(mbedtls_ssl_context *ssl);
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_load_buffered_message(mbedtls_ssl_context *ssl);
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_load_buffered_record(mbedtls_ssl_context *ssl);
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_buffer_message(mbedtls_ssl_context *ssl);
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_buffer_future_record(mbedtls_ssl_context *ssl,
                                    mbedtls_record const *rec);
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_next_record_is_in_datagram(mbedtls_ssl_context *ssl);

static inline  size_t ssl_get_maximum_datagram_size(mbedtls_ssl_context const *ssl)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_get_remaining_space_in_datagram(mbedtls_ssl_context const *ssl)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_get_remaining_payload_in_datagram(mbedtls_ssl_context const *ssl)
{
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_double_retransmit_timeout(mbedtls_ssl_context *ssl)
{
...
static inline  void ssl_reset_retransmit_timeout(mbedtls_ssl_context *ssl)
{
...
static inline  size_t ssl_compute_padding_length(size_t len,
                                         size_t granularity)
{
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_build_inner_plaintext(unsigned char *content,
                                     size_t *content_size,
                                     size_t remaining,
                                     uint8_t rec_type,
                                     size_t pad)
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_parse_inner_plaintext(unsigned char const *content,
                                     size_t *content_size,
                                     uint8_t *rec_type)
{
 ...
 *
 */
static inline  void ssl_extract_add_data_from_record(unsigned char *add_data,
                                             size_t *add_data_len,
                                             mbedtls_record *rec,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_transform_aead_dynamic_iv_is_explicit(
 ...
 */
static inline  void ssl_build_record_nonce(unsigned char *dst_iv,
                                   size_t dst_iv_len,
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_flight_append(mbedtls_ssl_context *ssl)
{
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_swap_epochs(mbedtls_ssl_context *ssl)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_hs_is_proper_fragment(mbedtls_ssl_context *ssl)
{
...
static inline  uint32_t ssl_get_hs_frag_len(mbedtls_ssl_context const *ssl)
{
    return MBEDTLS_GET_UINT24_BE(ssl->in_msg, 9);
}

static inline  uint32_t ssl_get_hs_frag_off(mbedtls_ssl_context const *ssl)
{
    return MBEDTLS_GET_UINT24_BE(ssl->in_msg, 6);
}

MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_check_hs_header(mbedtls_ssl_context const *ssl)
{
 ...
 */
static inline  void ssl_bitmask_set(unsigned char *mask, size_t offset, size_t len)
{
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_bitmask_check(unsigned char *mask, size_t len)
{
    size_t i;
...
static inline  size_t ssl_get_reassembly_buffer_size(size_t msg_len,
                                             unsigned add_bitmap)
{
    size_t alloc_len;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int mbedtls_ssl_dtls_record_replay_check(mbedtls_ssl_context *ssl, uint8_t *record_in_ctr)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_handle_possible_reconnect(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t len = 0;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_check_record_type(uint8_t record_type)
{
...
#if defined(MBEDTLS_SSL_DTLS_CLIENT_PORT_REUSE) && defined(MBEDTLS_SSL_SRV_C)
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_check_client_reconnect(mbedtls_ssl_context *ssl)
{
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_prepare_record_content(mbedtls_ssl_context *ssl,
                                      mbedtls_record *rec)
{
    int ret, done = 0;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_consume_current_message(mbedtls_ssl_context *ssl);
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_get_next_record(mbedtls_ssl_context *ssl);
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_record_is_in_progress(mbedtls_ssl_context *ssl);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_buffer_make_space(mbedtls_ssl_context *ssl,
                                 size_t desired)
{
    int offset;
    mbedtls_ssl_handshake_params * const hs = ssl->handshake;
    ...
    switch (ssl->in_msgtype) {
        ...
        case MBEDTLS_SSL_MSG_HANDSHAKE:
        {
            ...
            if (!hs_buf->is_valid) {
                ...
                hs_buf->data = (unsigned char*)mbedtls_calloc(1, reassembly_buf_sz);
                if (hs_buf->data == NULL) {
                                    ...
                                    mbedtls_record const *rec)
{
    ...
    hs->buffering.future_record.data =(unsigned char*)
...
static inline  size_t ssl_transform_get_explicit_iv_len(
    mbedtls_ssl_transform const *transform)
{
    return transform->ivlen - transform->fixed_ivlen;
}
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_check_ctr_renegotiate(mbedtls_ssl_context *ssl)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_tls13_is_new_session_ticket(mbedtls_ssl_context *ssl)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_tls13_handle_hs_message_post_handshake(mbedtls_ssl_context *ssl)
{
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_tls12_handle_hs_message_post_handshake(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_handle_hs_message_post_handshake(mbedtls_ssl_context *ssl)
{
 ...
 */
static inline  int ssl_read_application_data(
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static inline  int ssl_write_real(mbedtls_ssl_context *ssl,
 ...
 */
static inline  void ssl_buffering_shift_slots(mbedtls_ssl_context *ssl,
                                      unsigned shift)
{
    mbedtls_ssl_handshake_params * const hs = ssl->handshake;
    unsigned offset;
