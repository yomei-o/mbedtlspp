 ...
 * 0xFFFFFF52. */
static inline  psa_storage_uid_t psa_its_identifier_of_slot(mbedtls_svc_key_id_t key)
{
...
static inline int psa_is_key_present_in_storage(const mbedtls_svc_key_id_t key)
{
    psa_status_t ret;
    psa_storage_uid_t data_identifier = psa_its_identifier_of_slot(key);
    struct psa_storage_info_t data_identifier_info;
...
static inline void psa_format_key_data_for_storage(const uint8_t *data,
...
static inline psa_status_t psa_save_persistent_key(const psa_key_attributes_t *attr,
                                     const uint8_t *data,
                                     const size_t data_length)
{
    size_t storage_data_length;
...
static inline void psa_free_persistent_key_data(uint8_t *key_data, size_t key_data_length)
{
    mbedtls_zeroize_and_free(key_data, key_data_length);
}

static inline psa_status_t psa_load_persistent_key(psa_key_attributes_t *attr,
                                     uint8_t **data,
                                     size_t *data_length)
{
...
static inline psa_crypto_transaction_t psa_crypto_transaction;
